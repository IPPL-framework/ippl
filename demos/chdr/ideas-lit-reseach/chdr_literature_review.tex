...
This consolidated report combines the ChDR setup and beam parameters with
the single-electron literature review, the review of finite-difference
time-domain (FDTD) material interfaces, and the implications for the
{\tt ChDR} mini-app based on IPPL FEL. The electron remains in vacuum at a
gap \(a\) from the radiator. Entering the longitudinal region beside the
material is distinct from crossing into the dielectric.
...
\end{tikzpicture}
\caption{Schematic \(x\)--\(z\) view of the ChDR setup.
...
\section{Setup, fixed beam parameters and proof-of-concept geometry}
\label{sec:setup}
...
%\(10.0\unit{ps}\), consistent with the rounded setup value.
...
The beam-facing dielectric surface does not have to be tilted by the
approximately \(47^\circ\) internal Cherenkov angle. An axis-aligned slab is
the cleaner first material-interface test. It does not reproduce the final
out-coupling through a prism or the camera signal.
...
\subsection{Proof-of-concept scope and present obstacles}

There is no fundamental obstacle to the proposed proof-of-concept
calculation, but the following implementation issues delimit its first stage:
\begin{enumerate}[itemsep=4pt]
 \item The existing field formulation is a vacuum solver. A spatial
       constitutive response \(\epsilon_r(\mathbf r)\) and a verified
       vacuum--dielectric interface treatment must be added before the
       radiator is physical.
 \item The FEL mini-app uses a Lorentz-boosted frame, whereas a stationary
       isotropic radiator is most simply represented in the laboratory
       frame. The ChDR configuration therefore needs a laboratory-frame mode
       or a fully transformed material model.
 \item The existing second-order Mur boundary may reflect oblique radiation.
       It is suitable only when a boundary-convergence test shows that the
       reflected field is below the required tolerance; quantitative power
       calculations motivate a validated PML.
 \item A tilted prism on a Cartesian mesh introduces cut cells and possible
       staircasing. The first benchmark should use an axis-aligned planar
       interface, followed by subcell averaging or a conformal treatment for
       the extraction face.
\end{enumerate}
Sections~\ref{sec:fdtd} and~\ref{sec:ippl} develop these points and give a
validation sequence. The initial parameter scan is therefore the gap \(a\)
for a straight beam parallel to a flat radiator. The planar benchmark should
establish spectrum, polarization, gap dependence and the internal angle
before finite edges, the tilted prism and detector optics are introduced.
...
Kraus, Adelmann and Arbenz~\cite{kraus2011adipml} formulate PML for a
divergence-preserving alternating-direction implicit (ADI) Maxwell scheme. This is relevant when
...
The consolidated setup identifies dielectric interfaces, the reference frame
and absorbing boundaries as development topics for this thesis. The following
...
\(-5\unit{nC}\), the configured charge must be negative even though the beam
parameter table quotes its magnitude. A sketch of the relevant entries is
\begin{verbatim}
"charge": -3.1207545e10,
"mass":    3.1207545e10,
"gamma":   118.417071,
