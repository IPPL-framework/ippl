...
namespace initializer {
    ...
    void Output::grid2phys(real* pos_x, real* pos_y, real* pos_z, real* vel_x, real* vel_y,
                           real* vel_z, int Npart, int np, float rL) {
        long i;
    ...
    void Output::write_hcosmo_serial(real* pos_x, real* pos_y, real* pos_z, real* vel_x,
                                     real* vel_y, real* vel_z, int* id, int Npart, string outBase) {
        FILE* outFile;
    ...
    void Output::write_hcosmo_parallel(real* pos_x, real* pos_y, real* pos_z, real* vel_x,
                                       real* vel_y, real* vel_z, int* id, int Npart,
                                       string outBase) {
        FILE* outFile;
        ostringstream outName;
        long i;
    ...
    void Output::write_hcosmo_ascii(real* pos_x, real* pos_y, real* pos_z, real* vel_x, real* vel_y,
                                    real* vel_z, int Npart, string outBase) {
        long i;
        int MyPE, NumPEs, MasterPE, proc;
        MPI_Status status;
        ostringstream outName;
    ...
    void Output::write_gadget(InputParser& par, real* pos_x, real* pos_y, real* pos_z, real* vel_x,
                              real* vel_y, real* vel_z, int* id, int Npart, string outBase) {
        FILE* outFile;
        ostringstream outName;
        long i;
...
#define SKIP                                       \
    {                                              \
        fwrite(&blksize, sizeof(int), 1, outFile); \
    }
        blksize = sizeof(ghead);
        if (MyPE == MasterPE) {
            outFile = fopen(outName.str().c_str(), "wb");
            SKIP fwrite(&ghead, sizeof(ghead), 1, outFile);
            SKIP blksize = np * np * np * 3 * sizeof(float);
            SKIP for (i = 0; i < Npart; i++) {
