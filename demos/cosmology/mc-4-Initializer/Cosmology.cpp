...
namespace initializer {
    ...
    void CosmoClass::SetParameters(GlobalStuff& DataBase, const char* tfName) {
        ...
        if (TFFlag == 0) {  // CMBFast transfer function
            ...
            table_tf[0] = tfbar * Omega_bar / Omega_m + tfcdm * (Omega_m - Omega_bar) / Omega_m;
            norm        = table_tf[0];
            for (i = 1; i < table_size; ++i) {
                inputFile >> table_kk[i] >> tfcdm >> tfbar >> tmp >> tmp >> tmp >> tmp;
                table_tf[i] = tfbar * Omega_bar / Omega_m + tfcdm * (Omega_m - Omega_bar) / Omega_m;
                table_tf[i] =
                    table_tf[i]
                    / norm;  // Normalization
            ...
            */
            ...
            alpha = pow(akh1, -1.0 * Omega_bar / Omega_m)
                    * pow(akh2, pow(-1.0 * Omega_bar / Omega_m, 3.0));
            ...
            alpha_nu = alpha_nu
                       * (1.0
                          + (p_c - p_cb) / 2. * (1. + 1. / (3. - 4. * p_c) / (7. - 4. * p_cb))
                                / (1. + y_d));
            ...
            sound_horizon =
                2. / 3. / k_equality * sqrt(6. / R_equality)
...
#define pi M_PI
    ...
    real CosmoClass::eisenstein_hu(real k) {
        ...
        gamma_eff =
            omhh
            * (sqrt(alpha_nu) + (1. - sqrt(alpha_nu)) / (1. + pow(0.43 * k * sound_horizon, 4.0)));
        ...
        t_f   = t_f
              * (1.
                 + (1.2 * pow(f_nu, 0.64) * pow(N_nu, 0.3 + 0.6 * f_nu))
    ...
    real CosmoClass::klypin_holtzmann(real k) {
     ...
     This was taken care of at the beginning of the routine. */
        t_f = log(1.0 + 2.34 * qkh) / (2.34 * qkh)
              * pow(1.0 + 13.0 * qkh + pow(10.5 * qkh, 2.0) + pow(10.4 * qkh, 3.0)
                        + pow(6.51 * qkh, 4.0),
                    -0.25);
        return (t_f);
    }
    ...
    real CosmoClass::hu_sugiyama(real k) {
     ...
     This was taken care of at the beginning of the routine. */
        t_f = log(1.0 + 2.34 * qkh) / (2.34 * qkh)
              * pow(1.0 + 3.89 * qkh + pow(16.1 * qkh, 2.0) + pow(5.46 * qkh, 3.0)
                        + pow(6.71 * qkh, 4.0),
                    -0.25);
        return (t_f);
    }
...
#undef pi
    ...
    void CosmoClass::growths(real a, real y[], real dydx[]) {
        real H;
        H       = sqrt(Omega_r / (a * a * a * a) + Omega_m / (a * a * a)
                       + (1.0 - Omega_m - Omega_r) * pow(a, -3.0 * (1.0 + w_de)));
        dydx[0] = y[1] / (a * H);
...
#define FMIN(a, b) (minarg1 = (a), minarg2 = (b), (minarg1) < (minarg2) ? (minarg1) : (minarg2))
    void CosmoClass::rkqs(real y[], real dydx[], int n, real* x, real htry, real eps, real yscal[],
...
#undef FMIN
    ...
    void CosmoClass::rkck(real y[], real dydx[], int n, real x, real h, real yout[], real yerr[],
                          void (CosmoClass::*derivs)(real, real[], real[])) {
        int i;
        static real a2 = 0.2, a3 = 0.3, a4 = 0.6, a5 = 1.0, a6 = 0.875, b21 = 0.2, b31 = 3.0 / 40.0,
                    b32 = 9.0 / 40.0, b41 = 0.3, b42 = -0.9, b43 = 1.2, b51 = -11.0 / 54.0,
...
#define FUNC(x) ((this->*func)(x))
    real CosmoClass::midpoint(real (CosmoClass::*func)(real), real a, real b, int n) {
...
#define JMIN 5
    real CosmoClass::integrate(real (CosmoClass::*func)(real), real a, real b) {
        ...
        for (j = 1; j <= JMAX; ++j) {
            ...
            if (j > JMIN) {
                if (fabs(sol - old) < EPS * fabs(old) || (sol == 0.0 && old == 0.0))
                    return (sol);
            }
            old = sol;
        }
