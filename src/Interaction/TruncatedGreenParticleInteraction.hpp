...
namespace ippl {
    ...
    KOKKOS_INLINE_FUNCTION constexpr typename TruncatedGreenParticleInteraction<ParticleContainer, ScalarAttribute,
        ...
        return forceConstant * qm2 * (dist / r) * (
                   2.0 * alpha * Kokkos::exp(-alpha * alpha * r2) / (Kokkos::sqrt(Kokkos::numbers::pi) * r) +
    ...
    void TruncatedGreenParticleInteraction<ParticleContainer, ScalarAttribute, VectorAttribute>::solve() {
        ...
        particleLayout.template forEachPair<execution_space>(KOKKOS_LAMBDA(const size_t &i, const size_t &j) {
            const Vector_t dist_ij = R(i) - R(j);
            const Scalar_t rsq_ij = dist_ij.dot(dist_ij);

            if (rsq_ij >= rcut2) { return; }
