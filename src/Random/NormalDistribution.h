...
namespace ippl {
    namespace random {
        ...
        KOKKOS_FUNCTION T normal_pdf_func(T x, T mean, T stddev) {
            ...
            return (1.0 / (stddev * Kokkos::sqrt(2 * pi)))
                   * Kokkos::exp(-(x - mean) * (x - mean) / (2 * stddev * stddev));
        }
        ...
        /*!
         * @struct normal_cdf
         * @brief Functor to calculate the cumulative distribution function (CDF) for a normal
         * distribution.
         *
        ...
        /*!
         * @struct normal_pdf
         * @brief Functor to calculate the probability density function (PDF) for a normal
         * distribution.
         *
        ...
        struct NormalDistributionFunctions {
            // Functor to calculate the probability density function (PDF) for a normal
            // distribution.
            struct PDF {
                KOKKOS_INLINE_FUNCTION double operator()(T x, unsigned int d,
                                                         const T* params_p) const {
                    T mean   = params_p[2 * d + 0];
                    T stddev = params_p[2 * d + 1];
                    return ippl::random::normal_pdf_func<T>(x, mean, stddev);
                }
            ...
            // Functor to calculate the cumulative distribution function (CDF) for a normal
            // distribution.
            struct CDF {
                KOKKOS_INLINE_FUNCTION double operator()(T x, unsigned int d,
                                                         const T* params_p) const {
            ...
            struct Estimate {
                KOKKOS_INLINE_FUNCTION double operator()(T u, unsigned int d,
        ...
        class NormalDistribution
            : public ippl::random::Distribution<T, Dim, 2 * Dim, NormalDistributionFunctions<T>> {
        public:
            /*!
             * @brief Constructor for the Normal Distribution class.
             * The constructor takes an array of parameters of normal distribution, i.e. mean and
             * standard deviation.
             */
