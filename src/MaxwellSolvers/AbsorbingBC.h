 ...
 * @brief Implements a second-order absorbing boundary condition (ABC) for a face of a 3D computational domain.
 *
...
struct second_order_abc_face{
    ...
    scalar Cweights[5];                                     // Weights for the second-order ABC, precomputed based on mesh spacing and time step
    int sign;                                               // Sign of the main axis (1 for lower boundary, -1 for upper).
    constexpr static unsigned main_axis = _main_axis;       // Main axis normal to the face (0=x, 1=y, 2=z)
    ...
    KOKKOS_FUNCTION second_order_abc_face(ippl::Vector<scalar, 3> hr, scalar dt, int _sign) : sign(_sign){
        ...
        // Check that the main axis is one of the three axes and the side axes are different from each other and from the main axis
        ...
        Cweights[2]  	= ( 1.0 / ( c * dt * dt ) - c / (2.0 * hr[side_axes[0]] * hr[side_axes[0]]) - c / (2.0 * hr[side_axes[1]] * hr[side_axes[1]])) / d;
    ...
    KOKKOS_INLINE_FUNCTION auto operator()(const view_type& A_n, const view_type& A_nm1,const view_type& A_np1, const Coords& c)const -> typename view_type::value_type{
        ...
        ippl::Vector<uint32_t, 3> mainaxis_off = ippl::Vector<int32_t, 3>{(main_axis == 0) * sign, (main_axis == 1) * sign, (main_axis == 2) * sign};

        // Apply the boundary condition
        // The main axis is the one that is normal to the face, so we need to apply the boundary condition in the direction of the main axis,
        // this means we need to offset the coordinates by the sign of the main axis
 ...
 * @brief Implements a second-order absorbing boundary condition (ABC) for an edge of a 3D computational domain.
 *
 * This struct computes and applies the second-order Mur ABC on a specified edge of the domain,
 * minimizing reflections for electromagnetic field solvers. The edge is defined by its axis (the edge direction)
 * and two normal axes (the directions normal to the edge).
 *
 * The struct precomputes weights based on mesh spacing and time step, and provides an operator()
 * to update the field at the boundary using current, previous, and next time-step values.
 *
 ...
 * @tparam na1_zero       Boolean indicating direction for normal_axis1 (true for lower boundary, false for upper).
 * @tparam na2_zero       Boolean indicating direction for normal_axis2 (true for lower boundary, false for upper).
 */
template<typename _scalar, unsigned edge_axis, unsigned normal_axis1, unsigned normal_axis2, bool na1_zero, bool na2_zero>
struct second_order_abc_edge{
    using scalar = _scalar; // Define the scalar type for the weights and calculations
    scalar Eweights[5];     // Weights for the second-order ABC, precomputed based on mesh spacing and time step
    ...
    KOKKOS_FUNCTION second_order_abc_edge(ippl::Vector<scalar, 3> hr, scalar dt){
        // Check that the edge axis is one of the three axes and the normal axes are different from each other and from the edge axis
    ...
    KOKKOS_INLINE_FUNCTION auto operator()(const view_type& A_n, const view_type& A_nm1,const view_type& A_np1, const Coords& c)const -> typename view_type::value_type{
        ...
        // Get the opposite direction of the normal vectors for the two normal axes (direction to the interior of the domain)
 ...
 * @brief Implements a second-order absorbing boundary condition (ABC) for a corner of a 3D computational domain.
 *
 * This struct computes and applies the second-order Mur ABC on a specified corner of the domain,
 * minimizing reflections for electromagnetic field solvers. The corner is defined by its three axes,
 * with each axis having a boolean indicating whether it is at the lower or upper boundary.
 *
 * The struct precomputes weights based on mesh spacing and time step, and provides an operator()
 * to update the field at the boundary using current, previous, and next time-step values.
 *
 * @tparam _scalar  Scalar type (e.g., float or double).
 * @tparam x0       Boolean indicating direction for x-axis (true for lower boundary, false for upper).
 * @tparam y0       Boolean indicating direction for y-axis (true for lower boundary, false for upper).
 * @tparam z0       Boolean indicating direction for z-axis (true for lower boundary, false for upper).
 */
template<typename _scalar, bool x0, bool y0, bool z0>
struct second_order_abc_corner{
    using scalar = _scalar; // Define the scalar type for the weights and calculations
    scalar Cweights[17];    // Weights for the second-order ABC, precomputed based on mesh spacing and time step
 ...
 * @brief Applies second-order Mur absorbing boundary conditions (ABC) to all boundaries of a 3D computational domain.
 *
 * This struct coordinates the application of second-order Mur ABCs on faces, edges, and corners of the domain,
 * minimizing reflections for electromagnetic field solvers. It uses the appropriate face, edge, or corner ABC
 * implementation depending on the location of each boundary point.
 *
 * The apply() method detects the type of boundary (face, edge, or corner) for each point and applies the
 * corresponding second-order ABC using precomputed weights based on mesh spacing and time step.
 *
