...
namespace ippl {
    ...
    /**
     * @copydoc SubFieldLayout::SubFieldLayout(mpi::Communicator, const NDIndex<Dim>&, const
     * NDIndex<Dim>&, std::array<bool, Dim>, bool)
     *
    ...
    /**
     * @copydoc SubFieldLayout::SubFieldLayout(mpi::Communicator, const NDIndex<Dim>&,
     * std::array<bool, Dim>, bool)
     *
     * Implementation details:
    ...
    /**
     * @copydoc SubFieldLayout::initialize(const NDIndex<Dim>&, const NDIndex<Dim>&,
     * std::array<bool, Dim>, bool)
     *
     * Implementation details:
     ...
     */
    template <unsigned Dim>
    void SubFieldLayout<Dim>::initialize(const NDIndex<Dim>& domain, const NDIndex<Dim>& subDomain,
                                         std::array<bool, Dim> isParallel, bool isAllPeriodic) {
        // Ensure the sub-domain is contained within the main domain
        PAssert(domain.contains(subDomain));

        // Call the base class initialize method to set up the main domain and parallel
        // decomposition
        ...
        if (this->hLocalDomains_m(this->comm.rank()).intersect(subDomain).empty()) {
            throw std::runtime_error(
                ...
                + std::to_string(this->comm.rank())
                + " has an empty local domain, choose a sub-domain that has content on all ranks");
        }

        // If the local domain is not contained in the sub-domain, change it to the intersection of
     ...
     */
    template <unsigned Dim>
    void SubFieldLayout<Dim>::initialize(const NDIndex<Dim>& domain,
                                         std::array<bool, Dim> isParallel, bool isAllPeriodic) {
        // Call the base class initialize method to set up the main domain and parallel
        // decomposition
        FieldLayout<Dim>::initialize(domain, isParallel, isAllPeriodic);
