...
namespace ippl {
    ...
    template <class... Args>
    KOKKOS_FUNCTION
    NDIndex<Dim>::NDIndex(const Args&... args)
    : NDIndex({args...})
    {
    ...
    template <unsigned Dim>
    KOKKOS_FUNCTION
    NDIndex<Dim>::NDIndex(std::initializer_list<Index> indices) {
        unsigned int i = 0;
    ...
    const Index& NDIndex<Dim>::operator[](unsigned d) const noexcept {
        return indices_m[d];
    }
    ...
    template <unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    Index& NDIndex<Dim>::operator[](unsigned d) noexcept {
        return indices_m[d];
    }
    ...
    template <unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    unsigned NDIndex<Dim>::size() const noexcept {
    ...
    template<unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    bool NDIndex<Dim>::empty() const  noexcept {
        bool r = false;
    ...
    template <unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    NDIndex<Dim> NDIndex<Dim>::intersect(const NDIndex<Dim>& ndi) const
    {
        NDIndex<Dim> r;
        for (unsigned d = 0; d < Dim; ++d)
    ...
    template <unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    NDIndex<Dim> NDIndex<Dim>::grow(int ncells, unsigned int dim) const
    {
        NDIndex<Dim> r = *this;
    ...
    template<unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    bool NDIndex<Dim>::split(NDIndex<Dim>& l, NDIndex<Dim>& r, unsigned d) const
    {
        if ( &l != this )
            l = *this;
        if ( &r != this )
    ...
    template<unsigned Dim>
    KOKKOS_INLINE_FUNCTION
    bool NDIndex<Dim>::split(NDIndex<Dim>& l, NDIndex<Dim>& r) const
    {
