...
void Timing::print() {
...

    {
        ...
        MPI_Reduce(&tptr->wallTime, &walltotal, 1, MPI_DOUBLE, MPI_MAX, 0, ippl::Comm->getCommunicator());
    ...
    for (unsigned int i = 1; i < TimerList.size(); ++i) {
        ...
        double wallavg = 0.0;
        MPI_Reduce(&tptr->wallTime, &wallmax, 1, MPI_DOUBLE, MPI_MAX, 0, ippl::Comm->getCommunicator());
        MPI_Reduce(&tptr->wallTime, &wallmin, 1, MPI_DOUBLE, MPI_MIN, 0, ippl::Comm->getCommunicator());
        MPI_Reduce(&tptr->wallTime, &wallavg, 1, MPI_DOUBLE, MPI_SUM, 0, ippl::Comm->getCommunicator());
        size_t lengthName = std::min(tptr->name.length(), 19lu);
