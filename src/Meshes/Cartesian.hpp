...
void Cartesian<Dim, MFLOAT>::setup() {
...
//===========2D============
template <unsigned Dim, class MFLOAT>
Cartesian<Dim, MFLOAT>::Cartesian(const Index& I, const Index& J) {
...
//===========2D============
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::initialize(const Index& I, const Index& J) {
    PInsist(Dim == 2, "Number of Index arguments does not match mesh dimension!!");
    gridSizes[0] = I.length();  // Number of vertices along this dimension.
    gridSizes[1] = J.length();  // Number of vertices along this dimension.
    setup();                    // Setup chores, such as array allocations
...
void Cartesian<Dim, MFLOAT>::set_origin(const Vektor<MFLOAT, Dim>& o) {
    ...
    for (unsigned d = 0; d < Dim; ++d) {
        ...
        for (unsigned vert = 1; vert < gridSizes[d]; ++vert) {
            (meshPosition[d])[vert] = (meshPosition[d])[vert - 1] + (meshSpacing[d])[vert - 1];
        }
    }
...
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::get_meshSpacing(unsigned d, MFLOAT* spacings) const {
    PAssert_LT(d, Dim);
...
template <class T>
inline void PETE_apply(OpMeshPeriodic<T> /*e*/, T& a, T b) {
    a = b;
}
...
// 1D
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::storeSpacingFields(e_dim_tag p1, int vnodes) {
    e_dim_tag et[1];
    et[0] = p1;
...
// 2D
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::storeSpacingFields(e_dim_tag p1, e_dim_tag p2, int vnodes) {
    e_dim_tag et[2];
    et[0] = p1;
    et[1] = p2;
    storeSpacingFields(et, vnodes);
...
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::storeSpacingFields(e_dim_tag* et, int vnodes) {
    unsigned int d;
    ...
    if (!hasSpacingFields) {
        ...
        VertSpacings = new BareField<Vektor<MFLOAT, Dim>, Dim>(*FlCell, GuardCellSizes<Dim>(1));
        FlVert       = new FieldLayout<Dim>(verts, et, vnodes);
        // Note: enough guard cells only for existing Div(), etc. implementations:
        CellSpacings = new BareField<Vektor<MFLOAT, Dim>, Dim>(*FlVert, GuardCellSizes<Dim>(1));
    }
    ...
    typename BareField<Vektor<MFLOAT, Dim>, Dim>::iterator cfi, cfi_end = vertSpacings.end();
    ...
    typename BareField<Vektor<MFLOAT, Dim>, Dim>::iterator vfi, vfi_end = cellSpacings.end();
    for (vfi = cellSpacings.begin(); vfi != vfi_end; ++vfi) {
        vfi.GetCurrentLocation(currentLocation);
        for (d = 0; d < Dim; d++)
            cellSpacing(d) =
                0.5
                * ((meshSpacing[d])[currentLocation[d]] + (meshSpacing[d])[currentLocation[d] - 1]);
        *vfi = cellSpacing;
    }
    //-------------------------------------------------
    ...
    v0 = 0.0;
    v1 = 1.0;                                         // Used for Reflective mesh BC
    ...
    for (unsigned int face = 0; face < 2 * Dim; face++) {
        ...
        if (face & 1) {
            cSlab[d] = Index(verts[d].max() + 1, verts[d].max() + cellSpacings.rightGuard(d));
            vSlab[d] = Index(cells[d].max() + 1, cells[d].max() + vertSpacings.rightGuard(d));
        } else {
            cSlab[d] = Index(verts[d].min() - cellSpacings.leftGuard(d), verts[d].min() - 1);
            vSlab[d] = Index(cells[d].min() - vertSpacings.leftGuard(d), cells[d].min() - 1);
        }
        ...
        // +++++++++++++++cellSpacings++++++++++++++
        for (cfill_i = cellSpacings.begin_if(); cfill_i != cellSpacings.end_if(); ++cfill_i) {
            ...
            if (cSlab.touches(fill_alloc)) {
                ...
                for (from_i = cellSpacings.begin_if(); from_i != cellSpacings.end_if(); ++from_i) {
                    // Cache a few things.
                        ...
                        if (bct == Periodic) {
                            ...
                            if (bct == Reflective) {
                                BrickExpression<Dim, LFI, LFI, OpMeshExtrapolate<T> >(
                                    lhs, rhs, OpMeshExtrapolate<T>(v0, v1))
                                    .apply();
                            } else {
                                if (bct == NoBC) {
                                    BrickExpression<Dim, LFI, LFI, OpMeshExtrapolate<T> >(
                                        lhs, rhs, OpMeshExtrapolate<T>(v0, v0))
                                        .apply();
                                }
                            }
                        }
        ...
        // +++++++++++++++vertSpacings++++++++++++++
        for (vfill_i = vertSpacings.begin_if(); vfill_i != vertSpacings.end_if(); ++vfill_i) {
            // Cache some things we will use often below.
            // Pointer to the data for the current LField (right????):
            ...
            if (vSlab.touches(fill_alloc)) {
                ...
                // Loop over the ones that src touches.
                typename BareField<T, Dim>::iterator_if from_i;
                for (from_i = vertSpacings.begin_if(); from_i != vertSpacings.end_if(); ++from_i) {
                    // Cache a few things.
                    LField<T, Dim>& from           = *(*from_i).second;
                    const NDIndex<Dim>& from_owned = from.getOwned();
                    const NDIndex<Dim>& from_alloc = from.getAllocated();
...
void Cartesian<Dim, MFLOAT>::storeSpacingFields(e_dim_tag* p, unsigned* vnodesPerDirection,
    ...
    if (!hasSpacingFields) {
        // allocate layouts and spacing fields
        FlCell = new FieldLayout<Dim>(cells, p, vnodesPerDirection, recurse, vnodes);
        // Note: enough guard cells only for existing Div(), etc. implementations:
        VertSpacings = new BareField<Vektor<MFLOAT, Dim>, Dim>(*FlCell, GuardCellSizes<Dim>(1));
        FlVert       = new FieldLayout<Dim>(verts, p, vnodesPerDirection, recurse, vnodes);
        // Note: enough guard cells only for existing Div(), etc. implementations:
        CellSpacings = new BareField<Vektor<MFLOAT, Dim>, Dim>(*FlVert, GuardCellSizes<Dim>(1));
    }
    // VERTEX-VERTEX SPACINGS:
...
void Cartesian<Dim, MFLOAT>::print(std::ostream& out) {
    ...
    for (d = 0; d < Dim; d++) {
        ...
        for (mi = meshSpacing[d].begin(); mi != meshSpacing[d].end(); ++mi) {
            out << "meshSpacing[" << d << "][" << (*mi).first << "] = " << (*mi).second
                << std::endl;
        }
    }
    for (unsigned b = 0; b < (1 << Dim); b++)
...
template <unsigned Dim, class MFLOAT>
Field<MFLOAT, Dim, Cartesian<Dim, MFLOAT>, Cell>& Cartesian<Dim, MFLOAT>::getCellVolumeField(
    Field<MFLOAT, Dim, Cartesian<Dim, MFLOAT>, Cell>& volumes) const {
    ...
    typename Field<MFLOAT, Dim, Cartesian<Dim, MFLOAT>, Cell>::iterator fi, fi_end = volumes.end();
...
NDIndex<Dim> Cartesian<Dim, MFLOAT>::getNearestVertex(const Vektor<MFLOAT, Dim>& x) const {
    ...
    NDIndex<Dim> ndi;
    for (d = 0; d < Dim; d++)
        ndi[d] = Index(vertNearest[d], vertNearest[d], 1);
...
Cartesian<Dim, MFLOAT>::getVertexPositionField(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Vert>& vertexPositions) const {
    int currentLocation[Dim];
    Vektor<MFLOAT, Dim> vertexPosition;
...
Cartesian<Dim, MFLOAT>::getCellPositionField(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>& cellPositions) const {
    int currentLocation[Dim];
    Vektor<MFLOAT, Dim> cellPosition;
    ...
    for (fi = cellPositions.begin(); fi != fi_end; ++fi) {
        ...
        for (unsigned int d = 0; d < Dim; d++) {
            cellPosition(d) = 0.5
                              * ((*(meshPosition[d].find(currentLocation[d]))).second
...
template <unsigned Dim, class MFLOAT>
Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>&
Cartesian<Dim, MFLOAT>::getDeltaVertexField(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>& vertexSpacings) const {
    int currentLocation[Dim];
...
Vektor<MFLOAT, Dim> Cartesian<Dim, MFLOAT>::getDeltaCell(const NDIndex<Dim>& ndi) const {
    ...
    for (unsigned int d = 0; d < Dim; d++) {
        ...
        while (a <= b) {
            cellCellSpacing[d] +=
                ((*(meshSpacing[d].find(a))).second + (*(meshSpacing[d].find(a - 1))).second) * 0.5;
            a++;
        }
    }
...
template <unsigned Dim, class MFLOAT>
Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Vert>&
Cartesian<Dim, MFLOAT>::getDeltaCellField(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Vert>& cellSpacings) const {
    int currentLocation[Dim];
    ...
    for (fi = cellSpacings.begin(); fi != fi_end; ++fi) {
        ...
        for (unsigned int d = 0; d < Dim; d++)
            cellCellSpacing[d] += ((*(meshSpacing[d].find(currentLocation[d]))).second
                                   + (*(meshSpacing[d].find(currentLocation[d] - 1))).second)
                                  * 0.5;
        *fi = cellCellSpacing;
    }
    return cellSpacings;
...
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::getSurfaceNormalFields(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>** surfaceNormalsFields) const {
    Vektor<MFLOAT, Dim>* surfaceNormals = new Vektor<MFLOAT, Dim>[2 * Dim];
    unsigned int d, i;
    for (d = 0; d < Dim; d++) {
        for (i = 0; i < Dim; i++) {
    ...
    for (d = 0; d < 2 * Dim; d++)
        assign((*(surfaceNormalsFields[d])), surfaceNormals[d]);
...
template <unsigned Dim, class MFLOAT>
Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>&
Cartesian<Dim, MFLOAT>::getSurfaceNormalField(
    Field<Vektor<MFLOAT, Dim>, Dim, Cartesian<Dim, MFLOAT>, Cell>& surfaceNormalField,
    unsigned face) const {
    Vektor<MFLOAT, Dim> surfaceNormal;
    unsigned int d;
    // The following bitwise AND logical test returns true if face is odd
...
// One face at a time:
template <unsigned Dim, class MFLOAT>
void Cartesian<Dim, MFLOAT>::set_MeshBC(unsigned face, MeshBC_E meshBCType) {
        ...
        switch (MeshBC[d * 2]) {
            case Periodic:
                for (guardLayer = 0; guardLayer <= (gridSizes[d] - 1) / 2; guardLayer++) {
                    ...
                    (meshPosition[d])[cell + 1] = (meshPosition[d])[cell] + (meshSpacing[d])[cell];
                }
                break;
            case Reflective:
                for (guardLayer = 0; guardLayer <= (gridSizes[d] - 1) / 2; guardLayer++) {
            ...
            case NoBC:
                ...
                break;
            default:
                ERRORMSG("Cartesian::updateMeshSpacingGuards(): unknown MeshBC type" << endl);
                break;
        }
    } else {
        // "Low" guard cells:
        switch (MeshBC[d]) {
            case Periodic:
                for (guardLayer = 0; guardLayer <= (gridSizes[d] - 1) / 2; guardLayer++) {
                    ...
                    (meshPosition[d])[cell] = (meshPosition[d])[cell + 1] - (meshSpacing[d])[cell];
                }
                break;
            case Reflective:
                for (guardLayer = 0; guardLayer <= (gridSizes[d] - 1) / 2; guardLayer++) {
...
Field<T, 1U, Cartesian<1U, MFLOAT>, Cell>& Div(
    ...
    Index I                                         = domain[0];
    r[I] = dot(x[I], x.get_mesh().Dvc[0] / vertSpacings[I])
...
Field<T, 2U, Cartesian<2U, MFLOAT>, Cell>& Div(
    ...
    Index I                                         = domain[0];
    Index J                                         = domain[1];
    r[I][J] = dot(x[I][J], x.get_mesh().Dvc[0] / vertSpacings[I][J])
...
Field<T, 3U, Cartesian<3U, MFLOAT>, Cell>& Div(
    ...
    Index J                                         = domain[1];
    Index K                                         = domain[2];
    r[I][J][K] = dot(x[I][J][K], x.get_mesh().Dvc[0] / vertSpacings[I][J][K])
...
Field<T, 1U, Cartesian<1U, MFLOAT>, Vert>& Div(
    ...
    const NDIndex<1U>& domain                       = r.getDomain();
    Index I                                         = domain[0];
    r[I] = dot(x[I - 1], x.get_mesh().Dvc[0] / cellSpacings[I])
...
Field<T, 2U, Cartesian<2U, MFLOAT>, Vert>& Div(
    ...
    Index I                                         = domain[0];
    Index J                                         = domain[1];
    r[I][J] = dot(x[I - 1][J - 1], x.get_mesh().Dvc[0] / cellSpacings[I][J])
...
Field<T, 3U, Cartesian<3U, MFLOAT>, Vert>& Div(
    ...
    Index J                                         = domain[1];
    Index K                                         = domain[2];
    r[I][J][K] = dot(x[I - 1][J - 1][K - 1], x.get_mesh().Dvc[0] / cellSpacings[I][J][K])
...
Field<T, 2U, Cartesian<2U, MFLOAT>, Vert>& Div(
    ...
    idy[0]  = 0.0;
    idy[1]  = 1.0;
    r[I][J] = dot(idx, (x[I + 1][J] - x[I - 1][J]) / (vS[I][J] + vS[I - 1][J]))
...
Field<T, 3U, Cartesian<3U, MFLOAT>, Vert>& Div(
    ...
    idz[1]     = 0.0;
    idz[2]     = 1.0;
    r[I][J][K] = dot(idx, ((x[I + 1][J][K] - x[I - 1][J][K]) / (vS[I][J][K] + vS[I - 1][J][K])))
                 + dot(idy, ((x[I][J + 1][K] - x[I][J - 1][K]) / (vS[I][J][K] + vS[I][J - 1][K])))
                 + dot(idz, ((x[I][J][K + 1] - x[I][J][K - 1]) / (vS[I][J][K] + vS[I][J][K - 1])));
    return r;
}
//----------------------------------------------------------------------
...
Field<T, 2U, Cartesian<2U, MFLOAT>, Cell>& Div(
    ...
    idy[0]  = 0.0;
    idy[1]  = 1.0;
    r[I][J] = dot(idx, (x[I + 1][J] - x[I - 1][J]) / (cS[I][J] + cS[I - 1][J]))
...
Field<T, 3U, Cartesian<3U, MFLOAT>, Cell>& Div(
    ...
    idz[1]     = 0.0;
    idz[2]     = 1.0;
    r[I][J][K] = dot(idx, ((x[I + 1][J][K] - x[I - 1][J][K]) / (cS[I][J][K] + cS[I - 1][J][K])))
                 + dot(idy, ((x[I][J + 1][K] - x[I][J - 1][K]) / (cS[I][J][K] + cS[I][J - 1][K])))
                 + dot(idz, ((x[I][J][K + 1] - x[I][J][K - 1]) / (cS[I][J][K] + cS[I][J][K - 1])));
    return r;
}
//----------------------------------------------------------------------
...
Field<Vektor<T, 1U>, 1U, Cartesian<1U, MFLOAT>, Cell>& Grad(
    ...
    const NDIndex<1U>& domain                       = r.getDomain();
    Index I                                         = domain[0];
    r[I] = (x[I] * x.get_mesh().Dvc[0] + x[I + 1] * x.get_mesh().Dvc[1]) / vertSpacings[I];
    return r;
}
//----------------------------------------------------------------------
template <class T, class MFLOAT>
Field<Vektor<T, 2u>, 2U, Cartesian<2U, MFLOAT>, Cell>& Grad(
    ...
    Index I                                         = domain[0];
    Index J                                         = domain[1];
    r[I][J] = (x[I][J] * x.get_mesh().Dvc[0] + x[I + 1][J] * x.get_mesh().Dvc[1]
               + x[I][J + 1] * x.get_mesh().Dvc[2] + x[I + 1][J + 1] * x.get_mesh().Dvc[3])
              / vertSpacings[I][J];
    return r;
}
//----------------------------------------------------------------------
...
Field<Vektor<T, 3U>, 3U, Cartesian<3U, MFLOAT>, Cell>& Grad(
    ...
    Index K                                         = domain[2];
    r[I][J][K] =
        (x[I][J][K] * x.get_mesh().Dvc[0] + x[I + 1][J][K] * x.get_mesh().Dvc[1]
         + x[I][J + 1][K] * x.get_mesh().Dvc[2] + x[I + 1][J + 1][K] * x.get_mesh().Dvc[3]
         + x[I][J][K + 1] * x.get_mesh().Dvc[4] + x[I + 1][J][K + 1] * x.get_mesh().Dvc[5]
         + x[I][J + 1][K + 1] * x.get_mesh().Dvc[6] + x[I + 1][J + 1][K + 1] * x.get_mesh().Dvc[7])
        / vertSpacings[I][J][K];
    return r;
}
//----------------------------------------------------------------------
...
Field<Vektor<T, 1U>, 1U, Cartesian<1U, MFLOAT>, Vert>& Grad(
    ...
    const NDIndex<1U>& domain                       = r.getDomain();
    Index I                                         = domain[0];
    r[I] = (x[I - 1] * x.get_mesh().Dvc[0] + x[I] * x.get_mesh().Dvc[1]) / cellSpacings[I];
    return r;
}
//----------------------------------------------------------------------
template <class T, class MFLOAT>
Field<Vektor<T, 2U>, 2U, Cartesian<2U, MFLOAT>, Vert>& Grad(
    ...
    Index I                                         = domain[0];
    Index J                                         = domain[1];
    r[I][J] = (x[I - 1][J - 1] * x.get_mesh().Dvc[0] + x[I][J - 1] * x.get_mesh().Dvc[1]
               + x[I - 1][J] * x.get_mesh().Dvc[2] + x[I][J] * x.get_mesh().Dvc[3])
              / cellSpacings[I][J];
    return r;
}
//----------------------------------------------------------------------
...
Field<Vektor<T, 3U>, 3U, Cartesian<3U, MFLOAT>, Vert>& Grad(
    ...
    for (unsigned int d = 0; d < 1 << 3U; d++)
        dvc[d] = x.get_mesh().Dvc[d];
    r[I][J][K] =
        (x[I - 1][J - 1][K - 1] * dvc[0] + x[I][J - 1][K - 1] * dvc[1] + x[I - 1][J][K - 1] * dvc[2]
         + x[I][J][K - 1] * dvc[3] + x[I - 1][J - 1][K] * dvc[4] + x[I][J - 1][K] * dvc[5]
         + x[I - 1][J][K] * dvc[6] + x[I][J][K] * dvc[7])
        / cellSpacings[I][J][K];
    return r;
}
...
Field<Vektor<T, 2U>, 2U, Cartesian<2U, MFLOAT>, Vert>& Grad(
    ...
    r[I][J] = idx * ((x[I + 1][J] - x[I - 1][J]) / (vertSpacings[I][J] + vertSpacings[I - 1][J]))
              + idy * ((x[I][J + 1] - x[I][J - 1]) / (vertSpacings[I][J] + vertSpacings[I][J - 1]));
    return r;
}
//----------------------------------------------------------------------
template <class T, class MFLOAT>
...
Field<Vektor<T, 2u>, 2U, Cartesian<2U, MFLOAT>, Cell>& Grad(
    ...
    r[I][J] = idx * ((x[I + 1][J] - x[I - 1][J]) / (cellSpacings[I][J] + cellSpacings[I + 1][J]))
              + idy * ((x[I][J + 1] - x[I][J - 1]) / (cellSpacings[I][J] + cellSpacings[I][J + 1]));
    return r;
}
//----------------------------------------------------------------------
template <class T, class MFLOAT>
...
Field<Tenzor<T, 1U>, 1U, Cartesian<1U, MFLOAT>, Cell>& Grad(
    ...
    const NDIndex<1U>& domain             = r.getDomain();
    Index I                               = domain[0];
    r[I]                                  = outerProduct(x[I], x.get_mesh().Dvc[0] / vS[I])
...
Field<Tenzor<T, 2U>, 2U, Cartesian<2U, MFLOAT>, Cell>& Grad(
    ...
    Index I                               = domain[0];
    Index J                               = domain[1];
    r[I][J]                               = outerProduct(x[I][J], x.get_mesh().Dvc[0] / vS[I][J])
...
Field<Tenzor<T, 3U>, 3U, Cartesian<3U, MFLOAT>, Cell>& Grad(
    ...
    Index J                               = domain[1];
    Index K                               = domain[2];
    r[I][J][K] = outerProduct(x[I][J][K], x.get_mesh().Dvc[0] / vS[I][J][K])
...
Field<Tenzor<T, 1U>, 1U, Cartesian<1U, MFLOAT>, Vert>& Grad(
    ...
    const NDIndex<1U>& domain                       = r.getDomain();
    Index I                                         = domain[0];
    r[I] = outerProduct(x[I - 1], x.get_mesh().Dvc[0] / cellSpacings[I - 1])
...
Field<Tenzor<T, 2U>, 2U, Cartesian<2U, MFLOAT>, Vert>& Grad(
    ...
    Index I                               = domain[0];
    Index J                               = domain[1];
    r[I][J] = outerProduct(x[I - 1][J - 1], x.get_mesh().Dvc[0] / cS[I - 1][J - 1])
...
Field<Tenzor<T, 3U>, 3U, Cartesian<3U, MFLOAT>, Vert>& Grad(
    ...
    Index J                               = domain[1];
    Index K                               = domain[2];
    r[I][J][K] = outerProduct(x[I - 1][J - 1][K - 1], x.get_mesh().Dvc[0] / cS[I - 1][J - 1][K - 1])
...
namespace IPPL {
    ...
    Field<T1, 2U, Cartesian<2U, MFLOAT>, Vert>& Average(
        ...
        r[I][J] = (x[I - 1][J - 1] * w[I - 1][J - 1] + x[I][J - 1] * w[I][J - 1]
                   + x[I - 1][J] * w[I - 1][J] + x[I][J] * w[I][J])
    ...
    Field<T1, 3U, Cartesian<3U, MFLOAT>, Vert>& Average(
        ...
        r[I][J][K] =
            (x[I - 1][J - 1][K - 1] * w[I - 1][J - 1][K - 1]
             + x[I][J - 1][K - 1] * w[I][J - 1][K - 1] + x[I - 1][J][K - 1] * w[I - 1][J][K - 1]
             + x[I][J][K - 1] * w[I][J][K - 1] + x[I - 1][J - 1][K] * w[I - 1][J - 1][K]
             + x[I][J - 1][K] * w[I][J - 1][K] + x[I - 1][J][K] * w[I - 1][J][K]
             + x[I][J][K] * w[I][J][K])
            / (w[I - 1][J - 1][K - 1] + w[I][J - 1][K - 1] + w[I - 1][J][K - 1] + w[I][J][K - 1]
               + w[I - 1][J - 1][K] + w[I][J - 1][K] + w[I - 1][J][K] + w[I][J][K]);
        return r;
    }
    //----------------------------------------------------------------------
    ...
    Field<T1, 3U, Cartesian<3U, MFLOAT>, Vert>& Average(
        ...
        r[I][J][K] =
            0.125
            * (x[I - 1][J - 1][K - 1] + x[I][J - 1][K - 1] + x[I - 1][J][K - 1] + x[I][J][K - 1]
               + x[I - 1][J - 1][K] + x[I][J - 1][K] + x[I - 1][J][K] + x[I][J][K]);
        return r;
    }
    //----------------------------------------------------------------------
