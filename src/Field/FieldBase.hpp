...
namespace ippl {
    ...
    template <class FLayout, typename... IP>
    FieldBase<FLayout, IP...>::FieldBase()
        : layout_m(nullptr)
        , nextID_m(Comm->rank())
    ...
    template <class FLayout, typename... IP>
    FieldBase<FLayout, IP...>::FieldBase(FLayout& layout)
        : FieldBase() {
        initialize(layout);
    }
    ...
    void FieldBase<FLayout, IP...>::addAttribute(detail::FieldAttribBase<MemorySpace>& pa) {
        attributes_m.template get<MemorySpace>().push_back(&pa);
    }

    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::initialize(FLayout& layout) {
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::create(size_type nLocal) {
        PAssert(layout_m != nullptr);
        ...
        if (nLocal > 0) {
            ...
            if constexpr (EnableIDs) {
                // set the unique ID value for these new fields
                using policy_type =
                    Kokkos::RangePolicy<size_type, typename field_index_type::execution_space>;
                auto pIDs     = ID.getView();
                auto nextID   = this->nextID_m;
                auto numNodes = this->numNodes_m;
                Kokkos::parallel_for(
                    "FieldBase<...>::create(size_t)", policy_type(localNum_m, nLocal),
            ...
            // remember that we're creating these new fields
            localNum_m += nLocal;
        }
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::createWithID(index_type id) {
        PAssert(layout_m != nullptr);
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::globalCreate(size_type nTotal) {
        PAssert(layout_m != nullptr);
    ...
    void FieldBase<FLayout, IP...>::destroy(const Kokkos::View<bool*, Properties...>& invalid,
    ...
    template <class FLayout, typename... IP>
    template <typename... Properties>
    void FieldBase<FLayout, IP...>::internalDestroy(
        const Kokkos::View<bool*, Properties...>& invalid, const size_type destroyNum) {
        PAssert(destroyNum <= localNum_m);

        // If there aren't any fields to delete, do nothing
        if (destroyNum == 0) {
            return;
        }

        // If we're deleting all the fields, there's no point in doing
        ...
        // Find the indices of the invalid fields in the valid region
        Kokkos::parallel_scan(
            "Scan in FieldBase::destroy()", policy_type(0, localNum_m - destroyNum),
        ...
        // Determine the total number of invalid field in the valid region
        size_type maxDeleteIndex = 0;
        Kokkos::parallel_reduce(
            "Reduce in FiledBase::destroy()", policy_type(0, destroyNum),
        ...
        // Find the indices of the valid field in the invalid region
        Kokkos::parallel_scan(
            "Second scan in FieldBase::destroy()",
        ...
        // We need to delete fields in all memory spaces. If there are any attributes not stored
    ...
    void FieldBase<FLayout, IP...>::sendToRank(int rank, int tag, int sendNum,
        ...
        detail::runForAllSpaces([&]<typename MemorySpace>() {
            ...
            auto buf = Comm->getBuffer<MemorySpace>(IPPL_FIELD_SEND + sendNum, bufSize);
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::recvFromRank(int rank, int tag, int recvNum,
                                                    size_type nRecvs) {
        detail::runForAllSpaces([&]<typename MemorySpace>() {
            ...
            auto buf = Comm->getBuffer<MemorySpace>(IPPL_FIELD_RECV + recvNum, bufSize);
    ...
    void FieldBase<FLayout, IP...>::serialize(Archive& ar, size_type nsends) {
    ...
    template <class FLayout, typename... IP>
    template <typename Archive>
    void FieldBase<FLayout, IP...>::deserialize(Archive& ar, size_type nrecvs) {
        using memory_space = typename Archive::buffer_type::memory_space;
        forAllAttributes<memory_space>([&]<typename Attribute>(Attribute& att) {
    ...
    template <class FLayout, typename... IP>
    template <typename MemorySpace>
    detail::size_type FieldBase<FLayout, IP...>::packedSize(const size_type count) const {
        size_type total = 0;
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::pack(const hash_container_type& hash) {
        detail::runForAllSpaces([&]<typename MemorySpace>() {
    ...
    template <class FLayout, typename... IP>
    void FieldBase<FLayout, IP...>::unpack(size_type nrecvs) {
        detail::runForAllSpaces([&]<typename MemorySpace>() {
            auto& att = attributes_m.template get<MemorySpace>();
            for (unsigned j = 0; j < att.size(); j++) {
                att[j]->unpack(nrecvs);
