...
/***********************************************************************
      This is the user visible Kokkos_Field of type T.
...
//   Kokkos_Field(Layout_t &,const GuardCellSizes<Dim>&);
//   Kokkos_Field(Layout_t &,const BConds<T,Dim,M,C>&);
//   Kokkos_Field(Layout_t &,const GuardCellSizes<Dim>&,const BConds<T,Dim,M,C>&);
//   Kokkos_Field(Layout_t &,const BConds<T,Dim,M,C>&,const GuardCellSizes<Dim>&);
//   Kokkos_Field(FieldSpec<T,Dim,M,C>&);
//   constexpr Kokkos_Field(Kokkos_Field<T,Dim,M,C>&) = default;

  // Constructors including a Mesh object as argument:
  Kokkos_Field(Mesh_t&, Layout_t &);
//   Kokkos_Field(Mesh_t&, Layout_t &, const GuardCellSizes<Dim>&);
//   Kokkos_Field(Mesh_t&, Layout_t &, const BConds<T,Dim,M,C>&);
//   Kokkos_Field(Mesh_t&, Layout_t &, const GuardCellSizes<Dim>&,
// 	const BConds<T,Dim,M,C>&);
//   Kokkos_Field(Mesh_t&, Layout_t &, const BConds<T,Dim,M,C>&,
// 	const GuardCellSizes<Dim>&);
//   Kokkos_Field(Mesh_t&, FieldSpec<T,Dim,M,C>&);
...
//   bcond_iterator         end_BConds()           { return Bc.end(); }
  ...
  // When we apply a bracket it converts the type to Indexed_Field so
  ...
  // which we can use to index a Kokkos_Field in order to get a reference to
  // a subset of that Kokkos_Field.
//   Indexed_Field<T,Dim,1,M,C>   operator[](const Index&);
//   Indexed_Field<T,Dim,1,M,C>   operator[](int);
//   Indexed_Field<T,Dim,Dim,M,C> operator[](const NDIndex<Dim>&);
...
//   template<class X>
//   const Kokkos_Field<T,Dim,M,C>& operator=(const BareField<X,Dim>& x) {
//     assign(*this,x);
//     return *this;
//   }

  template<class B>
  const Kokkos_Field<T,Dim,M,C>& operator=(const PETE_Expr<B>& x) {
    assign(*this,x);
    return *this;
  }
...
//
//   //
//   // virtual functions for Kokkos_FieldLayoutUser's (and other UserList users)
//   //
//
