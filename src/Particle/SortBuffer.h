...
namespace ippl {
    namespace detail {
        ...
        /**
         * @brief Persistent scratch buffers shared by the counting-sort-based
         *        particle-to-bin grouping (Tiled / OutputFocused) and by the
         *        Morton-code particle sort (ParticleSort.h).
         *
         * Grows on demand, never shrinks. Call finalizeBinSortBuffers() before
         * Kokkos::finalize() to release device memory safely.
         *
         * Buffer layout
         *   bin_keys      uint64_t[n_particles]   per-particle bin / Morton code
         *   permute       size_t  [n_particles]   particle ids grouped by bin
         *   bin_offsets   size_t  [n_bins + 1]    start of each bin in permute
         *   cursor        size_t  [n_bins]        per-bin write cursor (counting-sort)
         *   keys_out      uint64_t[n_particles]   CUB radix-sort output (Morton sort)
         *   perm_out      size_t  [n_particles]   CUB radix-sort output (Morton sort)
         *   temp_storage  char    [temp_bytes]    CUB device-temp (Morton sort)
         */
        ...
        public:
            ...
            /**
             * @brief Ensure particle-sized and bin-sized buffers are large enough.
             *
            ...
            /**
             * @brief Ensure the CUB / rocPRIM temp-storage buffer is large enough.
             *        Used by the Morton-code radix sort path in ParticleSort.h.
             */
