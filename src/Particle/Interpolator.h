...
inline NDIndex<Dim> FindNGP(const M& mesh, const Vektor<PT, Dim>& ppos, CenteringTag<Cell>) {
    return mesh.getCellContaining(ppos);
}

template <class PT, unsigned Dim, class M>
inline NDIndex<Dim> FindNGP(const M& mesh, const Vektor<PT, Dim>& ppos, CenteringTag<Vert>) {
...
protected:
    ...
    static CompressedBrickIterator<T, Dim> getFieldIter(const BareField<T, Dim>& f,
        ...
        ERRORMSG(pt << " on node " << Comm->myNode() << endl);
