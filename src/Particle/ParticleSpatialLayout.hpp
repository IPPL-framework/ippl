...
namespace ippl {
    ...
    ParticleSpatialLayout<T, Dim, Mesh, Properties...>::ParticleSpatialLayout(FieldLayout<Dim>& fl,
         ...
         */
        rankSendCount_dview_m = locate_type("rankSendCount Device", nRanks);

        /* The indices have no particluar meaning,
         * the values are the MPI ranks to which we need to send
         */
        destinationRanks_dview_m = locate_type("destinationRanks Device", nRanks);
    }
    ...
    void ParticleSpatialLayout<T, Dim, Mesh, Properties...>::update(ParticleContainer& pc) {
        ...
        auto [nInvalid, nDestinationRanks] =
            locateParticles(pc, particleRanks_m, invalidParticles_m, rankSendCount_dview_m,
                            destinationRanks_dview_m);

        /* Host space copy of rankSendCount_dview */
        auto rankSendCount_hview =
            Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), rankSendCount_dview_m);

        /* Host Space copy of destinationRanks_dview */
        auto destinationRanks_hview =
            Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), destinationRanks_dview_m);
        ...
        for (size_t ridx = 0; ridx < nDestinationRanks; ridx++) {
            ...
            fillHash(rank, particleRanks_m, hash);
        ...
        pc.internalDestroy(invalidParticles_m, nInvalid);
        Kokkos::fence();
