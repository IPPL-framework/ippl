...
namespace ippl {
	    ...
	    FFTPoissonSolver<Tlhs, Trhs, Dim, M, C>::initializeFields() {
	        ...
	        mesh2_m = std::unique_ptr<M>(new M(domain2_m, hr_m, origin));
	        ...
	        meshComplex_m = std::unique_ptr<M>(new M(domainComplex_m, hr_m, origin));
	        layoutComplex_m = std::unique_ptr<FieldLayout_t>(new FieldLayout_t(domainComplex_m, decomp));
            ...
            int out = this->params_m.template get<int>("output_type");
            if (((out == Base::GRAD) || 
                (out == Base::SOL_AND_GRAD)) && (!isGradFD_m)) {
	        ...
	        if (alg_m == "HOCKNEY") {
                ...
                // start a timer
                static IpplTimings::TimerRef initialize_hockney = IpplTimings::getTimer("Initialize: extra Hockney");
	            ...
	            for (unsigned int d = 0; d < Dim; ++d) {
		            ...
		            switch (d) {
		                case 0:
		                    Kokkos::parallel_for("Helper index Green field initialization", 
    ...
    template <typename Tlhs, typename Trhs, unsigned Dim, class M, class C> 
	void
    FFTPoissonSolver<Tlhs, Trhs, Dim, M, C>::solve() {
        ...
        if (ranks > 1) {
            ...
            Kokkos::parallel_for("Write rho on the doubled grid",
                              mdrange_type({nghost1, nghost1, nghost1},
                              {view1.extent(0)-nghost1, view1.extent(1)-nghost1, view1.extent(2)-nghost1}), 
                              KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
        ...
        if ((out == Base::SOL) ||
            ...
            if (ranks > 1) {
                ...
                Kokkos::parallel_for("Write the solution into the LHS on physical grid",
                              mdrange_type({nghost1, nghost1, nghost1},
                              {view1.extent(0)-nghost1, view1.extent(1)-nghost1, view1.extent(2)-nghost1}),
                              KOKKOS_LAMBDA(const int i, const int j, const int k) {
        ...
        //if output_type is GRAD or SOL_AND_GRAD, we caculate E-field (gradient in Fourier domain)
        if (((out == Base::GRAD) || 
           (out == Base::SOL_AND_GRAD)) && (!isGradFD_m)) {
            ...
            for (size_t gd = 0; gd < Dim; ++gd) {
                ...
                Kokkos::parallel_for("Gradient - E field",
                               mdrange_type({nghostR, nghostR, nghostR},
                               {viewR.extent(0)-nghostR, viewR.extent(1)-nghostR, viewR.extent(2)-nghostR}),
                               KOKKOS_LAMBDA(const int i, const int j, const int k) {
                ...
                // start a timer
                static IpplTimings::TimerRef edtos = IpplTimings::getTimer("Efield: double to phys.");
                ...
                if (ranks > 1) {
                    ...
                    Kokkos::parallel_for("Write the E-field on physical grid",
                              mdrange_type({nghostL, nghostL, nghostL},
                              {viewL.extent(0)-nghostL, viewL.extent(1)-nghostL, viewL.extent(2)-nghostL}),
                              KOKKOS_LAMBDA(const int i, const int j, const int k) {
    ...
    FFTPoissonSolver<Tlhs, Trhs, Dim, M, C>::greensFunction() {
        ...
        if (alg_m == "VICO") {
            ...
            if (ranks > 1) {
                ...
                Kokkos::parallel_for("Restrict domain of Green's function from 4N to 2N",
                              mdrange_type({nghost, nghost, nghost},
            ...
            using mdrange_type = Kokkos::MDRangePolicy<Kokkos::Rank<3>>;
            Kokkos::parallel_for("Regularize Green's function ",
                              mdrange_type({nghost, nghost, nghost},
                              {view.extent(0)-nghost, view.extent(1)-nghost, view.extent(2)-nghost}),
                              KOKKOS_LAMBDA(const int i, const int j, const int k) {
