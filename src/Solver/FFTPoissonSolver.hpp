        ...
        if (ranks > 1) {
            ...
            for (int i = 0; i < ranks; ++i) {
                if (lDomains2[i].touches(ldom1)) {
                    ...
                    buffer_type buf =
                        Ippl::Comm->getBuffer<Trhs, memory_space>(IPPL_SOLVER_SEND + i, nsends);
            ...
            for (int i = 0; i < ranks; ++i) {
                if (lDomains1[i].touches(ldom2)) {
                    ...
                    buffer_type buf = Ippl::Comm->getBuffer<Trhs, memory_space>(
        ...
        for (int i = 0; i < ranks; ++i) {
            ...
            if (domain2.touches(none)) {
                ...
                if (ldom_g.touches(intersection)) {
                    ...
                    buffer_type buf =
                        Ippl::Comm->getBuffer<Trhs, memory_space>(IPPL_VICO_SEND + i, nsends);
            ...
            if (domain2.touches(x)) {
                ...
                if (ldom_g.touches(domain4)) {
                    ...
                    buffer_type buf =
                        Ippl::Comm->getBuffer<Trhs, memory_space>(IPPL_VICO_SEND + 8 + i, nsends);
