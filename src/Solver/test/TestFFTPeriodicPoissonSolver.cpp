    ...
    for (int p=0; p<npts; ++p) {
        ...
        switch (params.template get<int>("output_type")) {
            case Solver_t::SOL: {
                ...
                Kokkos::parallel_for("Assign rhs",
                                                ...
                                                const int k)
                {
                    ...
                    //view(i, j, k) = 3.0 * pow(pi, 2) * sin(pi * x) * sin(pi * y) * sin(pi * z);
                    ...
                    view_exact(i, j, k) = sin(sin(pi * x)) * sin(sin(pi * y)) * sin(sin(pi * z));
                });
                ...
                if(Ippl::Comm->rank() == 0) {
                    std::cout << "L2 relative error norm: "
                              << error_norm2 << std::endl;
                }
                break;
                    }
            ...
            case Solver_t::GRAD: {
                                                    ...
                                                    const int k)
                {
                    ...
                    Eview_exact(i, j, k)[0] = -pi * cos(pi * x) * cos(sin(pi * x)) * sin(sin(pi * y)) * sin(sin(pi * z));
                    Eview_exact(i, j, k)[1] = -pi * cos(pi * y) * cos(sin(pi * y)) * sin(sin(pi * x)) * sin(sin(pi * z));
                    Eview_exact(i, j, k)[2] = -pi * cos(pi * z) * cos(sin(pi * z)) * sin(sin(pi * x)) * sin(sin(pi * y));
                });
                ...
                if(Ippl::Comm->rank() == 0) {
                    for (size_t d=0; d<dim; ++d) {
                    std::cout << "L2 relative error norm Efield["
                              << d
                              << "]: "
