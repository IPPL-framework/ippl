...
KOKKOS_INLINE_FUNCTION float gaussian(float x, float y, float z, float sigma = 0.05,
                                      float mu = 0.5) {
...
KOKKOS_INLINE_FUNCTION float exact_fct(float x, float y, float z, float sigma = 0.05,
                                       float mu = 0.5) {
    float pi = std::acos(-1.0);
...
KOKKOS_INLINE_FUNCTION ippl::Vector<float, 3> exact_E(float x, float y, float z, float sigma = 0.05,
                                                      float mu = 0.5) {
    float pi     = std::acos(-1.0);
    float r      = std::sqrt((x - mu) * (x - mu) + (y - mu) * (y - mu) + (z - mu) * (z - mu));
    float factor = (1.0 / (4.0 * pi * r * r))
...
void dumpVTK(std::string path,
             ippl::Field<float, 3, ippl::UniformCartesian<float, 3>,
                         ippl::UniformCartesian<float, 3>::DefaultCentering>& rho,
             int nx, int ny, int nz, int iteration, float dx, float dy, float dz) {
    typename ippl::Field<
        float, 3, ippl::UniformCartesian<float, 3>,
        ippl::UniformCartesian<float, 3>::DefaultCentering>::view_type::host_mirror_type host_view =
        rho.getHostMirror();
...
int main(int argc, char* argv[]) {
    ...
    for (int p = 0; p < n; ++p) {
        ...
        typedef ippl::Field<float, 3, Mesh_t, Centering_t> field;
        field rho;
        ...
        typedef ippl::Field<ippl::Vector<float, 3>, 3, Mesh_t, Centering_t> fieldV;
        ...
        Kokkos::parallel_for(
            "Assign exact E-field",
            Kokkos::MDRangePolicy<Kokkos::Rank<3>>(
                {nghost, nghost, nghost},
                {view_exactE.extent(0) - nghost, view_exactE.extent(1) - nghost,
        ...
        ippl::FFTPoissonSolver<ippl::Vector<float, 3>, float, 3, Mesh_t, Centering_t> FFTsolver(
