...
int main(int argc, char* argv[]) {
    ...
    typedef ippl::Field<float, dim, Mesh_t, Centering_t> field_lhs_type;
    field_type rhs(mesh, layout);
    field_lhs_type lhs(mesh, layout), solution(mesh, layout);

    typedef ippl::BConds<float, dim, Mesh_t, Centering_t> bc_type;

    bc_type bcField;

    for (unsigned int i = 0; i < 6; ++i) {
        bcField[i] = std::make_shared<ippl::PeriodicFace<float, dim, Mesh_t, Centering_t>>(i);
    }

    lhs.setFieldBC(bcField);

    typename field_type::view_type &viewRHS = rhs.getView();
    typename field_lhs_type::view_type viewSol = solution.getView();
    ...
    Kokkos::parallel_for(
        "Assign solution", policySol, KOKKOS_LAMBDA(const int i, const int j, const int k) {
            ...
            float x        = (ig + 0.5) * float( hx[0] );
            float y        = (jg + 0.5) * float( hx[1] );
            float z        = (kg + 0.5) * float( hx[2] );

            viewSol(i, j, k) = sin(sin(float(pi) * x)) * sin(sin(float(pi) * y)) * sin(sin(float(pi) * z));
        });
    ...
    ippl::ElectrostaticsCG<float, double, dim, Mesh_t, Centering_t> lapsolver;
    ...
    m << size << "," << std::setprecision(7) << relError << "," << residue << "," << itCount
      << endl;
