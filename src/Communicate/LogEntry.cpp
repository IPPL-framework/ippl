...
std::string deserializeString(const std::vector<char>& buffer, size_t& offset) {
    size_t length = deserializeBasicType<size_t>(buffer, offset); // Get the length of the string
    std::string str(buffer.begin() + offset, buffer.begin() + offset + length); // Extract the string
    offset += length;
    return str;
}
...
std::vector<char> LogEntry::serialize() const {
    ...
    serializeBasicType(buffer, allocatedSize);
...
LogEntry LogEntry::deserialize(const std::vector<char>& buffer, size_t offset) {
    ...
    entry.allocatedSize = deserializeBasicType<size_t>(buffer, current_pos);
