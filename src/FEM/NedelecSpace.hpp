...
namespace ippl {
    ...
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::NedelecSpace(
        UniformCartesian<T, Dim>& mesh, ElementType& ref_element, const QuadratureType& quadrature,
        const Layout_t& layout)
        : FiniteElementSpace<T, Dim, getNedelecNumElementDOFs(Dim, Order), ElementType,
                             QuadratureType, FEMVector<T>, FEMVector<T>>(mesh, ref_element,
                                                                         quadrature) {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    void NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::initialize(
        UniformCartesian<T, Dim>& mesh, const Layout_t& layout) {
        FiniteElementSpace<T, Dim, getNedelecNumElementDOFs(Dim, Order), ElementType,
                           QuadratureType, FEMVector<T>, FEMVector<T>>::setMesh(mesh);
        ...
        localDofPositions_m(2)(0)  = 0.5;
        localDofPositions_m(2)(1)  = 1;
        localDofPositions_m(3)(0)  = 1;
        localDofPositions_m(3)(1)  = 0.5;
        localDofPositions_m(4)(2)  = 0.5;
        localDofPositions_m(5)(0)  = 1;
        localDofPositions_m(5)(2)  = 0.5;
        localDofPositions_m(6)(0)  = 1;
        localDofPositions_m(6)(1)  = 1;
        localDofPositions_m(6)(2)  = 0.5;
        localDofPositions_m(7)(1)  = 1;
        localDofPositions_m(7)(2)  = 0.5;
        localDofPositions_m(8)(0)  = 0.5;
        localDofPositions_m(8)(2)  = 1;
        localDofPositions_m(9)(1)  = 0.5;
        localDofPositions_m(9)(2)  = 1;
        localDofPositions_m(10)(0) = 0.5;
        localDofPositions_m(10)(1) = 1;
        localDofPositions_m(10)(2) = 1;
        localDofPositions_m(11)(0) = 1;
        localDofPositions_m(11)(1) = 0.5;
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    void NedelecSpace<T, Dim, Order, ElementType, QuadratureType,
                      FieldType>::initializeElementIndices(const Layout_t& layout) {
        layout_m         = layout;
    ...
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::numGlobalDOFs() const {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    KOKKOS_FUNCTION size_t
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getLocalDOFIndex(
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    KOKKOS_FUNCTION size_t
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getGlobalDOFIndex(
    ...
    KOKKOS_FUNCTION Vector<
        size_t, NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::numElementDOFs>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getLocalDOFIndices()
        const {
    ...
    KOKKOS_FUNCTION Vector<
        size_t, NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::numElementDOFs>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getGlobalDOFIndices(
        const NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::indices_t&
            elementIndex) const {
        ...
        if constexpr (Dim == 3) {
            ...
            globalDOFs(4) = v(2) * elementIndex(2) + 2 * nx * ny - nx - ny + elementIndex(1) * nx
                            + elementIndex(0);
    ...
    KOKKOS_FUNCTION Vector<
        size_t, NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::numElementDOFs>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getFEMVectorDOFIndices(
        NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::indices_t elementIndex,
        ...
        if constexpr (Dim == 3) {
            ...
            FEMVectorDOFs(4) = v(2) * elementIndex(2) + 2 * nx * ny - nx - ny + elementIndex(1) * nx
                               + elementIndex(0);
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    KOKKOS_FUNCTION
        typename NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::point_t
        NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::getLocalDOFPosition(
            size_t localDOFIndex) const {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    template <typename F>
    FEMVector<T> NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::evaluateAx(
        FEMVector<T>& x, F& evalFunction) const {
        Inform m("");
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    FEMVector<T>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::evaluateLoadVector(
        const FEMVector<NedelecSpace<T, Dim, Order, ElementType, QuadratureType,
                                     FieldType>::point_t>& f) const {
        // List of quadrature weights
        const Vector<T, QuadratureType::numElementNodes> w =
            this->quadrature_m.getWeightsForRefElement();
        ...
        Vector<Vector<T, numElementDOFs>, QuadratureType::numElementNodes> quadratureDOFDistances;
        for (size_t k = 0; k < QuadratureType::numElementNodes; ++k) {
            for (size_t i = 0; i < numElementDOFs; ++i) {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    template <typename F>
    FEMVector<T> NedelecSpace<T, Dim, Order, ElementType, QuadratureType,
                              FieldType>::evaluateLoadVectorFunctor(const F& f) const {
        // List of quadrature weights
        const Vector<T, QuadratureType::numElementNodes> w =
            this->quadrature_m.getWeightsForRefElement();
    ...
    KOKKOS_FUNCTION
        ...
        NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::
            evaluateRefElementShapeFunction(
                ...
                const NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::point_t&
                    localPoint) const {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    FEMVector<T>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::createFEMVector() const {
    ...
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::reconstructToPoints(
        ...
        Kokkos::parallel_for(
            "reconstructToPoints", positions.extent(0), KOKKOS_CLASS_LAMBDA(size_t i) {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    KOKKOS_FUNCTION bool NedelecSpace<T, Dim, Order, ElementType, QuadratureType,
                                      FieldType>::isDOFOnBoundary(const size_t& dofIdx) const {
        ...
        if constexpr (Dim == 2) {
            ...
            // West
            onBoundary =
                onBoundary || ((dofIdx >= nx - 1) && (dofIdx - (nx - 1)) % (2 * nx - 1) == 0);
            // East
            onBoundary = onBoundary
                         || ((dofIdx >= 2 * nx - 2) && ((dofIdx - 2 * nx + 2) % (2 * nx - 1) == 0));
        }
        ...
        if constexpr (Dim == 3) {
            ...
            if (dofIdx - (nx * (ny - 1) + ny * (nx - 1) + nx * ny) * zOffset
                >= (nx * (ny - 1) + ny * (nx - 1))) {
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    KOKKOS_FUNCTION int NedelecSpace<T, Dim, Order, ElementType, QuadratureType,
                                     FieldType>::getBoundarySide(const size_t& dofIdx) const {
        if constexpr (Dim == 2) {
            size_t nx = this->nr_m[0];
            size_t ny = this->nr_m[1];
            ...
            if (dofIdx < nx - 1)
                return 0;
            // West
            if ((dofIdx - (nx - 1)) % (2 * nx - 1) == 0)
                return 1;
            // North
            if (dofIdx > nx * (ny - 1) + ny * (nx - 1) - nx)
                return 2;
            // East
            if ((dofIdx >= 2 * nx - 2) && (dofIdx - 2 * nx + 2) % (2 * nx - 1) == 0)
                return 3;
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    FEMVector<T>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::createFEMVector2d() const {
        ...
        for (size_t i = 0; i < neighbors.size(); ++i) {
            sendIdxs.push_back(Kokkos::View<size_t*>(
                "FEMvector::sendIdxs[" + std::to_string(i) + "]", sendIdxsTemp[i].size()));
            recvIdxs.push_back(Kokkos::View<size_t*>(
                "FEMvector::recvIdxs[" + std::to_string(i) + "]", recvIdxsTemp[i].size()));
    ...
    template <typename T, unsigned Dim, unsigned Order, typename ElementType,
              typename QuadratureType, typename FieldType>
    FEMVector<T>
    NedelecSpace<T, Dim, Order, ElementType, QuadratureType, FieldType>::createFEMVector3d() const {
        // Here we will create an empty FEMVector for the case of the domain
        // being 3D.
        ...
        auto flatBoundaryExchange = [this, &neighbors, &ldom](
                                        ...
                                        std::vector<std::vector<size_t>>& va,
                                        std::vector<std::vector<size_t>>& vb, int posA, int posB,
                                        const std::vector<size_t>& idxsA,
                                        const std::vector<size_t>& idxsB, NDIndex<3>& adom,
                                        NDIndex<3>& bdom) {
        ...
        auto negativeDiagonalExchange = [this, &neighbors, &ldom](
                                            ...
                                            std::vector<std::vector<size_t>>& va,
                                            std::vector<std::vector<size_t>>& vb,
                                            const std::vector<size_t>& idxsA,
                                            const std::vector<size_t>& idxsB, NDIndex<3>& odom) {
            neighbors.push_back(i);
            va.push_back(std::vector<size_t>());
            vb.push_back(std::vector<size_t>());
            size_t idx = neighbors.size() - 1;
        ...
        auto positiveDiagonalExchange = [this, &neighbors, &ldom](
                                            size_t i, size_t a, size_t f, size_t s, indices_t posA,
                                            indices_t posB, std::vector<std::vector<size_t>>& va,
                                            std::vector<std::vector<size_t>>& vb,
                                            const std::vector<size_t>& idxsA,
                                            const std::vector<size_t>& idxsB, NDIndex<3>& odom) {
            neighbors.push_back(i);
        ...
        for (size_t i = 0; i < doms.extent(0); ++i) {
            ...
            if (ldom.last()[0] == odom.first()[0] - 1
                ...
                int pos = ldom.last()[0];
                flatBoundaryExchange(i, 0, 1, 2, recvIdxsTemp, sendIdxsTemp, pos, pos,
                                     {3, 5, 6, 11}, {0, 1, 4, 2, 7, 8, 9, 10}, ldom, odom);
            }
            ...
            if (ldom.first()[0] == odom.last()[0] + 1
                ...
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                int pos = ldom.first()[0];
                flatBoundaryExchange(i, 0, 1, 2, sendIdxsTemp, recvIdxsTemp, pos, pos - 1,
                                     {1, 4, 7, 9}, {0, 1, 4, 2, 7, 8, 9, 10}, odom, ldom);
            }
            ...
            if (ldom.last()[1] == odom.first()[1] - 1
                ...
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                int pos = ldom.last()[1];
                flatBoundaryExchange(i, 1, 0, 2, recvIdxsTemp, sendIdxsTemp, pos, pos,
                                     {2, 7, 6, 10}, {0, 1, 4, 3, 5, 8, 9, 11}, ldom, odom);
            }
            ...
            if (ldom.first()[1] == odom.last()[1] + 1
                ...
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                int pos = ldom.first()[1];
                flatBoundaryExchange(i, 1, 0, 2, sendIdxsTemp, recvIdxsTemp, pos, pos - 1,
                                     {0, 4, 5, 8}, {0, 1, 4, 3, 5, 8, 9, 11}, odom, ldom);
            }
            ...
            if (ldom.last()[2] == odom.first()[2] - 1
                ...
                && !(odom.last()[1] < ldom.first()[1] || odom.first()[1] > ldom.last()[1])) {
                int pos = ldom.last()[2];
                flatBoundaryExchange(i, 2, 0, 1, recvIdxsTemp, sendIdxsTemp, pos, pos,
                                     {8, 9, 11, 10}, {0, 1, 4, 3, 5, 2, 7, 6}, ldom, odom);
            }
            ...
            if (ldom.first()[2] == odom.last()[2] + 1
                ...
                flatBoundaryExchange(i, 2, 0, 1, sendIdxsTemp, recvIdxsTemp, pos, pos - 1,
                                     {0, 1, 3, 2}, {0, 1, 4, 3, 5, 2, 7, 6}, odom, ldom);
            }
            ...
            if (ldom.last()[0] == odom.first()[0] - 1 && ldom.first()[2] == odom.last()[2] + 1
                && !(odom.last()[1] < ldom.first()[1] || odom.first()[1] > ldom.last()[1])) {
                negativeDiagonalExchange(i, 1, 0, 2, 0, -1, sendIdxsTemp, recvIdxsTemp, {0, 1},
                                         {3, 5}, odom);
            }
            ...
            if (ldom.first()[0] == odom.last()[0] + 1 && ldom.last()[2] == odom.first()[2] - 1
                && !(odom.last()[1] < ldom.first()[1] || odom.first()[1] > ldom.last()[1])) {
                negativeDiagonalExchange(i, 1, 2, 0, -1, 0, recvIdxsTemp, sendIdxsTemp, {8, 9},
                                         {1, 4}, odom);
            }
            ...
            if (ldom.last()[1] == odom.first()[1] - 1 && ldom.first()[2] == odom.last()[2] + 1
                && !(odom.last()[0] < ldom.first()[0] || odom.first()[0] > ldom.last()[0])) {
                negativeDiagonalExchange(i, 0, 1, 2, 0, -1, sendIdxsTemp, recvIdxsTemp, {0, 1},
                                         {2, 7}, odom);
            }
            ...
            if (ldom.first()[1] == odom.last()[1] + 1 && ldom.last()[2] == odom.first()[2] - 1
                && !(odom.last()[0] < ldom.first()[0] || odom.first()[0] > ldom.last()[0])) {
                negativeDiagonalExchange(i, 0, 2, 1, -1, 0, recvIdxsTemp, sendIdxsTemp, {8, 9},
                                         {0, 4}, odom);
            }
            ...
            if (ldom.last()[0] == odom.first()[0] - 1 && ldom.first()[1] == odom.last()[1] + 1
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                negativeDiagonalExchange(i, 2, 0, 1, 0, -1, sendIdxsTemp, recvIdxsTemp, {0, 4},
                                         {3, 5}, odom);
            }
            ...
            if (ldom.first()[0] == odom.last()[0] + 1 && ldom.last()[1] == odom.first()[1] - 1
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                negativeDiagonalExchange(i, 2, 1, 0, -1, 0, recvIdxsTemp, sendIdxsTemp, {2, 7},
                                         {1, 4}, odom);
            }
            ...
            if (ldom.last()[0] == odom.first()[0] - 1 && ldom.last()[2] == odom.first()[2] - 1
                && !(odom.last()[1] < ldom.first()[1] || odom.first()[1] > ldom.last()[1])) {
                positiveDiagonalExchange(i, 1, 0, 2, ldom.last(), ldom.last(), sendIdxsTemp,
                                         recvIdxsTemp, {0, 1, 4}, {11}, odom);
            }

            // Parallel to y from space to ground, east to west
            if (ldom.first()[0] == odom.last()[0] + 1 && ldom.first()[2] == odom.last()[2] + 1
                && !(odom.last()[1] < ldom.first()[1] || odom.first()[1] > ldom.last()[1])) {
                positiveDiagonalExchange(i, 1, 0, 2, ldom.first() - 1, ldom.first(), recvIdxsTemp,
                                         sendIdxsTemp, {0, 1, 4}, {1}, odom);
            }

            // Parallel to x from ground to space, south to north
            if (ldom.last()[1] == odom.first()[1] - 1 && ldom.last()[2] == odom.first()[2] - 1
                && !(odom.last()[0] < ldom.first()[0] || odom.first()[0] > ldom.last()[0])) {
                positiveDiagonalExchange(i, 0, 1, 2, ldom.last(), ldom.last(), sendIdxsTemp,
                                         recvIdxsTemp, {0, 1, 4}, {10}, odom);
            }

            // Parallel to x from space to ground, north to south
            if (ldom.first()[1] == odom.last()[1] + 1 && ldom.first()[2] == odom.last()[2] + 1
                && !(odom.last()[0] < ldom.first()[0] || odom.first()[0] > ldom.last()[0])) {
                positiveDiagonalExchange(i, 0, 1, 2, ldom.first() - 1, ldom.first(), recvIdxsTemp,
                                         sendIdxsTemp, {0, 1, 4}, {0}, odom);
            }

            // Parallel to z from west to east, south to north
            if (ldom.last()[0] == odom.first()[0] - 1 && ldom.last()[1] == odom.first()[1] - 1
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                positiveDiagonalExchange(i, 2, 0, 1, ldom.last(), ldom.last(), sendIdxsTemp,
                                         recvIdxsTemp, {0, 1, 4}, {6}, odom);
            }

            // Parallel to z from east to west, north to south
            if (ldom.first()[0] == odom.last()[0] + 1 && ldom.first()[1] == odom.last()[1] + 1
                && !(odom.last()[2] < ldom.first()[2] || odom.first()[2] > ldom.last()[2])) {
                positiveDiagonalExchange(i, 2, 0, 1, ldom.first() - 1, ldom.first(), recvIdxsTemp,
                                         sendIdxsTemp, {0, 1, 4}, {4}, odom);
            }
        }
        ...
        size_t nz = extents(2);
        size_t n =
            (nz - 1) * (nx * (ny - 1) + ny * (nx - 1) + nx * ny) + nx * (ny - 1) + ny * (nx - 1);
        FEMVector<T> vec(n, neighbors, sendIdxs, recvIdxs);
