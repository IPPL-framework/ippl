...
namespace ippl {
    ...
    public:
        ...
        typedef typename Element1D<T, NumVertices>::mesh_vertex_vec_t mesh_vertex_vec_t;
        typedef typename Element1D<T, NumVertices>::diag_matrix_vec_t diag_matrix_vec_t;

        mesh_vertex_vec_t getLocalVertices() const override;

        diag_matrix_vec_t getTransformationJacobian(
            const mesh_vertex_vec_t& global_vertices) const override;
