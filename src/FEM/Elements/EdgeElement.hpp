...
namespace ippl {
    ...
    template <typename T>
    EdgeElement<T>::mesh_vertex_vec_t EdgeElement<T>::getLocalVertices() const {
        EdgeElement::mesh_vertex_vec_t vertices;
        vertices[0] = {0.0};
        vertices[1] = {1.0};
        return vertices;
    }
    ...
    EdgeElement<T>::diag_matrix_vec_t EdgeElement<T>::getTransformationJacobian(
        const EdgeElement<T>::mesh_vertex_vec_t& global_vertices) const {
