namespace ippl {
    ...
    FEMVector<T>::FEMVector(size_t n, std::vector<size_t> neighbors,
        ...
        , boundaryInfo_m(
              new BoundaryInfo(std::move(neighbors), std::move(sendIdxs), std::move(recvIdxs))) {}

    template <typename T>
    FEMVector<T>::FEMVector(size_t n)
        : data_m("FEMVector::data", n)
        , boundaryInfo_m(nullptr) {}

    template <typename T>
    KOKKOS_FUNCTION FEMVector<T>::FEMVector(const FEMVector<T>& other)
        : data_m(other.data_m)
    ...
    void FEMVector<T>::setHalo(T setValue) {
        ...
        for (size_t i = 0; i < boundaryInfo_m->neighbors_m.size(); ++i) {
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_CLASS_LAMBDA(const size_t& j) { data_m[view(j)] = setValue; });
        }
    }
    ...
    FEMVector<T>& FEMVector<T>::operator=(T value) {
        Kokkos::parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA(const size_t& i) { data_m[i] = value; });
        return *this;
    }
    ...
    FEMVector<T>& FEMVector<T>::operator=(const detail::Expression<E, N>& expr) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA(const size_t& i) { data_m[i] = expr_(i); });
        return *this;
    }
    ...
    FEMVector<T>& FEMVector<T>::operator=(const FEMVector<T>& v) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA(const size_t& i) { data_m[i] = view(i); });
        return *this;
    }
    ...
    FEMVector<T> FEMVector<T>::deepCopy() const {
        ...
        if (boundaryInfo_m) {
            ...
            for (size_t i = 0; i < boundaryInfo_m->neighbors_m.size(); ++i) {
                newSendIdxs.emplace_back(
                    Kokkos::View<size_t*>(boundaryInfo_m->sendIdxs_m[i].label(),
                                          boundaryInfo_m->sendIdxs_m[i].extent(0)));
                Kokkos::deep_copy(newSendIdxs[i], boundaryInfo_m->sendIdxs_m[i]);

                newRecvIdxs.emplace_back(
                    Kokkos::View<size_t*>(boundaryInfo_m->recvIdxs_m[i].label(),
    ...
    void FEMVector<T>::pack(const Kokkos::View<size_t*>& idxStore) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA(const size_t& i) { bufferData(i) = data_m(idxStore(i)); });
        Kokkos::fence();
    }
    ...
    void FEMVector<T>::unpack(const Kokkos::View<size_t*>& idxStore) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA(const size_t& i) { op(data_m(idxStore(i)), bufferData(i)); });
        Kokkos::fence();
    }
