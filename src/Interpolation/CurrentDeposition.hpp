...
namespace ippl {
     ...
     */
    template <typename Mesh, typename ChargeAttrib, typename PosAttrib, typename JField,
              typename policy_type = Kokkos::RangePolicy<>>
    inline void assemble_current_collocated(const Mesh& mesh, const ChargeAttrib& q_attrib,
                                            const PosAttrib& X0, const PosAttrib& X1,
                                            JField& J_field, policy_type iteration_policy,
                                            typename Mesh::value_type dt) {
        using T                = typename Mesh::value_type;
        constexpr unsigned Dim = Mesh::Dimension;
        static_assert(Dim == 2 || Dim == 3, "assemble_current_collocated only supports 2D and 3D");
        ...
        Kokkos::parallel_for(
            "assemble_current_collocated", iteration_policy, KOKKOS_LAMBDA(const std::size_t p) {
                auto segs = GridPathSegmenter<Dim, T, DefaultCellCrossingRule>::split(X0(p), X1(p),
                                                                                      origin, h);
                ...
                for (unsigned i = 0; i < Dim + 1; ++i) {
                    ...
                    // ada: PR501 would remove this co-located current-deposition centering note.
     ...
     * For each particle p moving from X0(p) to X1(p) during one time step dt,
     * the trajectory is split into sub-segments each lying within a single mesh
     * cell (via GridPathSegmenter). For each spatial component, every segment's contribution to J
     * is scattered onto the 2^(Dim-1) Yee-grid nodes using linear interpolation weights evaluated
     * at the midpoint.
     *
     */
    template <typename Mesh, typename ChargeAttrib, typename PosAttrib, typename JField,
              typename policy_type = Kokkos::RangePolicy<>>
    inline void assemble_current_yee(const Mesh& mesh, const ChargeAttrib& q_attrib,
                                     const PosAttrib& X0, const PosAttrib& X1, JField& J_field,
                                     policy_type iteration_policy, typename Mesh::value_type dt) {
        using T                = typename Mesh::value_type;
        constexpr unsigned Dim = Mesh::Dimension;
        static_assert(Dim == 2 || Dim == 3, "assemble_current_yee only supports 2D and 3D");
        ...
        Kokkos::parallel_for(
            "assemble_current_yee", iteration_policy, KOKKOS_LAMBDA(const std::size_t p) {
                auto segs = GridPathSegmenter<Dim, T, DefaultCellCrossingRule>::split(X0(p), X1(p),
                                                                                      origin, h);
