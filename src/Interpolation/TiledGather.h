...
namespace detail {
    ...
    __global__ void tiled_gather_3d_cuda_kernel(
        ...
        Kokkos::View<ValueType*, Kokkos::CudaSpace> output,  // Output values
        int64_t n_points,
        int nghost,
        Vector<int, 3> n_grid_global,  // GLOBAL grid dimensions (for coordinate transformation)
        ...
        for (int d = 0; d < 3; ++d) {
            ...
            idx0_global[d] = grid_point_to_grid_idx(pos[d], n_grid_global[d], w) - (w - 1) / 2;
        }
        ...
        for (int linear_idx = lane_id; linear_idx < w3; linear_idx += WARP_SIZE) {
            ...
            thread_sum += field_view(gi_local + nghost, gj_local + nghost, gk_local + nghost) * kernel_val;
        }
        ...
        static void dispatch_3d(
            ...
            if constexpr (W <= MaxW) {
                if (w == W) {
                    ...
                    tiled_gather_3d_cuda_kernel<W, RealType, PositionViewType, PermuteViewType,
                                                FieldViewType, KernelType, ValueType>
                        <<<grid_size, block_size>>>(
                            x, permute, field_view, output,
                            n, nghost, n_grid_global, n_grid_local, local_offset,
                            inv_hw, kernel, add_to_attribute);
                } else {
                    CudaGatherDispatcher<W + 1, MaxW>::template dispatch_3d<RealType, PositionViewType,
                        PermuteViewType, FieldViewType, KernelType, ValueType>(
                        w, n, x, permute, field_view, output,
                        nghost, n_grid_global, n_grid_local, local_offset,
                        inv_hw, kernel, add_to_attribute);
                }
            } else {
