...
namespace detail {
    ...
    template<int w, typename RealType, typename PositionViewType, typename PermuteViewType,
             typename BinOffsetsViewType, typename ValueViewType, typename GridViewType, typename KernelType>
    __global__ void tiled_scatter_3d_cuda_kernel(
        ...
        const size_type tile_idx_1d = tile_x + tile_y * num_tiles_x + tile_z * num_tiles_x * num_tiles_y;
        ...
        for (size_type p_idx = threadIdx.x; p_idx < num_particles_in_tile; p_idx += blockDim.x) {
            ...
            for (int wz = 0; wz < z_count; ++wz) {
                ...
                for (int wy = 0; wy < w; ++wy) {
                    // #pragma unroll
                    for (int wx = 0; wx < w; ++wx) {
                        const int hist_idx = ((idx_z + wz) * hist_size_y + (idx_y + wy)) * hist_size_x + (idx_x + wx);
                        const RealType kernel_val = kernel_x[wx] * kernel_y[wy] * kernel_z[wz];

                        if constexpr (std::is_same_v<grid_element_type, Kokkos::complex<RealType>>) {
                            if constexpr (std::is_same_v<value_type, Kokkos::complex<RealType>>) {
        ...
        for (int idx = threadIdx.x; idx < hist_size; idx += blockDim.x) {
            ...
            if constexpr (std::is_same_v<grid_element_type, Kokkos::complex<RealType>>) {
                atomicAdd(&grid(global_x + nghost, global_y + nghost, global_z + nghost).real(), hist_val.real());
                atomicAdd(&grid(global_x + nghost, global_y + nghost, global_z + nghost).imag(), hist_val.imag());
            } else {
                atomicAdd(&grid(global_x + nghost, global_y + nghost, global_z + nghost), hist_val);
            }
        }
    }
        ...
        static void dispatch_3d(
            ...
            if constexpr (W <= MaxW) {
                if (w == W) {
                    ...
                    tiled_scatter_3d_cuda_kernel<W, RealType, PositionViewType, PermuteViewType,
                                                  BinOffsetsViewType, ValueViewType, GridViewType, KernelType>
                        <<<threads_per_tile * num_tiles_total, threads_per_block, shared_mem_bytes>>>(
                            x, permute, bin_offsets, values, grid,
                            tile_size_x, tile_size_y, tile_size_z, z_tiles,
                            n_grid[0], n_grid[1], n_grid[2], nghost, inv_hw, kernel);
                    ...
                    CudaScatterDispatcher<W + 1, MaxW>::template dispatch_3d<RealType, PositionViewType, PermuteViewType,
                                                                              BinOffsetsViewType, ValueViewType, GridViewType, KernelType>(
                        w, bin_offsets, permute, x, values, grid, n_grid, num_tiles,
                        tile_size_x, tile_size_y, tile_size_z, z_tiles, nghost, inv_hw, kernel);
                }
            } else {
        ...
        static void dispatch_3d(
            ...
            if constexpr (W <= MaxW) {
                if (w == W) {
                    ...
                    if constexpr (std::is_same_v<ExecSpace, Kokkos::Cuda>) {
                        CudaScatterDispatcher<W, W>::template dispatch_3d<RealType, PositionViewType, PermuteViewType,
                                                                           BinOffsetsViewType,
                                                                           Kokkos::View<ValueType*, typename ExecSpace::memory_space>,
                                                                           GridViewType, KernelType>(
                            w, bin_offsets, permute, x, values, grid, n_grid, num_tiles,
                            tile_size_x, tile_size_y, tile_size_z, z_tiles, nghost, inv_hw, kernel);
                    } else
#endif
