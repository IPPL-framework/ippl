...
typedef OrthogonalRecursiveBisection ORB;

template < class T, unsigned Dim, class M>
static bool 
ORB<T,Dim,M>::BinaryRepartition(ParticleBase<ParticleSpatialLayout<T,Dim,M> >& P) {
...
template < class T, unsigned Dim, class M>
void
ORB<T,Dim,M>::CalcBinaryRepartition(FieldLayout<Dim>& FL, Field<T, Dim>& BF) {
...
template < class T, unsigned Dim, class M>
int
ORB<T,Dim,M>::FindCutAxis(FieldLayout<Dim>& FL) {
   return 0;
} 
...
template < class T, unsigned Dim, class M>
void
ORB<T,Dim,M>::PerformReduction(Field<T, Dim> BF, std::vector<T>& res, int cutAxis) {
...
template < class T, unsigned Dim, class M>
int
ORB<T,Dim,M>::FindMedian(std::vector<T>& V) {
   return 0;
}
...
template < class T, unsigned Dim, class M>
void
ORB<T,Dim,M>::CutDomain(FieldLayout<Dim>& FL, int cutAxis, int median) {
