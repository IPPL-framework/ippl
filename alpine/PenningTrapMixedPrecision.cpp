...
KOKKOS_FUNCTION
double PDF(const Vector_t<float, Dim>& xvec, const Vector_t<float, Dim>& mu,
           const Vector_t<float, Dim>& sigma, const unsigned Dim) {
    double pdf = 1.0;
    double pi  = Kokkos::numbers::pi_v<double>;
...
int main(int argc, char* argv[]) {
    ...
    ippl::initialize(argc, argv);
    {
        ...
        Vector_t<float, Dim> rmin = 0;
        Vector_t<float, Dim> rmax = 20;

        Vector_t<double, Dim> hr     = rmax / nr;
        Vector_t<float, Dim> origin = rmin;
        unsigned int nrMax           = 2048;  // Max grid size in our studies
        float dxFinest              = rmax[0] / nrMax;
        const float dt              = 0.5 * dxFinest;  // size of timestep
        ...
        Vector_t<float, Dim> length = rmax - rmin;

        Vector_t<float, Dim> mu, sd;
        ...
        if ((P->loadbalancethreshold_m != 1.0) && (ippl::Comm->size() > 1)) {
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(const int i, const int j, const int k) {
                    ...
                    float x        = (ig + 0.5) * hr[0] + origin[0];
                    float y        = (jg + 0.5) * hr[1] + origin[1];
                    float z        = (kg + 0.5) * hr[2] + origin[2];

                    Vector_t<float, Dim> xvec = {x, y, z};
        ...
        float factor             = (Nr[0] * Nr[1] * Nr[2]) / (Dr[0] * Dr[1] * Dr[2]);
