...
struct SimulationParameters {
    Vector_t<int, Dim> meshRefinement;
...
bool parseArgs(int argc, char* argv[], SimulationParameters<Dim>& params) {
    ...
    while (true) {
        ...
        for (unsigned i = 0; i < Dim; i++) {
            ...
            longOpts[i + ARG_COUNT] = {name, required_argument, NULL, (int)(i + 1)};
        }
        ...
        switch (opt) {
            ...
            case 'd': {
                size_type cells =
                    std::reduce(params.meshRefinement.begin(), params.meshRefinement.end(), 1,
                                std::multiplies<size_type>());
                size_type ppc = std::atoll(optarg);
                size_type np  = ppc * cells;
                if (ppc == 0) {
                    *ippl::Warn << "Particle density specified as zero." << endl;
                } else if (np == 0) {
                    *ippl::Warn
                        << "Particle density was specified for a mesh with zero cells. Note that "
