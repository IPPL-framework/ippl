...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        for (size_t nv = nvMin; nv <= NV_MAX; nv *= 2) {
            ...
            ippl::e_dim_tag configSpaceDecomp[Dim] = {ippl::PARALLEL, ippl::PARALLEL,
                                                      ippl::PARALLEL};
            ...
            if constexpr (testType == TestCase::MAXWELLIAN) {
                ...
                sigma = 0.05 * vMax;
                // Scaling prefactor s.t. inital vel. density matches what we see at iteration 100
                // in DIH
            ...
            if (nv == 32) {
                dumpVTKScalar(P->fv_m, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Happr");
                dumpVTKScalar(HfieldExact, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Hexact");
                dumpVTKScalar(Hdiff, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Hdiff");
                dumpVTKVector(P->Fd_m, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Fdappr");
                dumpVTKVector(FdExact, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Fdexact");
                dumpVTKVector(FdDiff, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "FdDiff");
            }
            ...
            if (nv == 32) {
                ...
                dumpVTKScalar(GfieldExact, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Gexact");
                dumpVTKScalar(Gdiff, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Gdiff");
            }
            ...
            if (nv == 32) {
                dumpVTKScalar(Dtrace, P->hvInit_m, P->nv_m, P->vminInit_m, nv, 1.0, OUT_DIR,
                              "Dtrace");
