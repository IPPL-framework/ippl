...
Matrix_t choleskyb( V& d0, V& d1, V& d2){
        ...
        Matrix_t LL;
        V* D[] = {&d0, &d1, &d2}; 
        double epszero = __DBL_EPSILON__;
...
int main(){
    ...
    Vector_t D0, D1, D2;
    Matrix_t L, L2, L_sol;
    double avgerror=0, error=0, error2=0;

    std::ifstream fin;
    std::ofstream fout;

    fin.open("./cholesky_test_data/np_out.csv");
    ...
    for(int i = 0; i < 200; ++i){
        ...
        //         // fout << L[c][d] <<"=?"<<  L_sol[c][d] << "   ";
        //         fout << std::abs(L[c][d] - L_sol[c][d])/ L_sol[c][d] << "   ";
        //     }
        // } 
        // fout << std::endl;
        ...
        for(int d = 0; d < 3 ; ++d){
                ...
                // error2 += std::abs( (L2[0][d] - D0[d]) /D0[d] );
                // error2 += std::abs( (L2[1][d] - D1[d]) /D1[d] );
                // error2 += std::abs( (L2[2][d] - D2[d]) /D2[d] );
                fout << std::abs( (L2[d][0] - D0[d]) /D0[d] ) << " "
                     << std::abs( (L2[d][1] - D1[d]) /D1[d] ) << " "
                     << std::abs( (L2[d][2] - D2[d]) /D2[d] ) << " " << std::endl;
        } 
        fout << std::endl;
    ...
    // avgerror  /= 6*200;
    // std::cout << "average relative error(sum of relative errors over all elements of L) over 200stof tests is:  " << std::endl;
