...
class P3MHeatingManager : public P3M3DManager<T, Dim, FieldContainer<T, Dim> > {
public:
...
public:
    ...
    void pre_run() override {
        ...
        this->setFieldSolver(std::make_shared<P3MSolver_t<T, Dim> >(
        ...
        this->setInteractionSolver(std::make_shared<typename Base::PPInteraction>(
            *this->pcontainer_m, this->pcontainer_m->E, this->pcontainer_m->R,
            this->pcontainer_m->Q, ppInteractionParams));
    ...
    void computeBeamStatistics() {
        ...
        const T emit_x =
            Kokkos::sqrt(global_stats[0] * global_stats[1] - global_stats[2] * global_stats[2]);
        const T emit_y =
            Kokkos::sqrt(global_stats[3] * global_stats[4] - global_stats[5] * global_stats[5]);
        const T emit_z =
            Kokkos::sqrt(global_stats[6] * global_stats[7] - global_stats[8] * global_stats[8]);
    ...
    void computeTemperature() {
        ...
        Kokkos::parallel_reduce(
            "compute average velocity", nLoc,
            KOKKOS_LAMBDA(const size_type i, Vector_t<T, 3>& sum) { sum += P(i); }, avgVel);
        Kokkos::fence();
    ...
    void initializeParticles() {
        ...
        Kokkos::parallel_for(
            "initialize particles", nloc, KOKKOS_LAMBDA(const size_t index) {
                ...
                Vector_t<T, Dim> pos =
                    beamRad * (Kokkos::pow(u, 1. / 3.) / Kokkos::sqrt(normsq)) * x;
    ...
    void pre_step() override { Inform m("pre step"); }
    ...
    T computeAvgSpaceChargeForces() {
        ...
        Kokkos::parallel_reduce(
            "compute average space charge forces", nLoc,
    ...
    void applyConstantFocusing() {
        ...
        Kokkos::parallel_for(
            "apply constant focusing", nLoc, KOKKOS_LAMBDA(const size_type& i) {
