#ifndef IPPL_FEL_VIDEO_WRITER_H
#define IPPL_FEL_VIDEO_WRITER_H

// Poynting-flux visualization for the FEL simulation.
//
// Renders a slice of the (lab-frame) Poynting field plus the projected particle
// positions to a BMP frame and pipes the stream to ffmpeg. Ported from the
// original FreeElectronLaser.cpp main loop and encapsulated here so the manager
// and driver stay readable. stb_image_write's implementation is emitted by the
// translation unit that defines STB_IMAGE_WRITE_IMPLEMENTATION before including
// this header (FreeElectronLaser.cpp).
...
// Google "Turbo" colormap (Anton Mikhailov), 256 RGB entries in [0,1].
inline constexpr float turbo_cm[256][3] = {
...
// Write an RGB buffer as a BMP to an open FILE* (the ffmpeg pipe).
inline bool writeBMPToFD(FILE* fd, int width, int height, const unsigned char* data) {
...
// Encapsulates the ffmpeg pipe and per-frame rendering.
template <typename T, unsigned Dim>
class FELVideoWriter {
public:
    // Open the ffmpeg pipe on rank 0 when periodic output is requested.
    void open(const config& cfg) {
        if (ippl::Comm->rank() == 0 && cfg.output_rhythm != 0) {
            ...
            ffmpeg_file_ = popen(ffmpegCmd, "w");
        }
    }

    void close() {
        if (ffmpeg_file_ != nullptr) {
            pclose(ffmpeg_file_);
            ffmpeg_file_ = nullptr;
        }
    }

    // Render and emit one frame for time step `it`. Collective over all ranks
    // (the per-rank partial images are reduced onto rank 0 before encoding).
    template <typename FieldContainer_t, typename ParticleContainer_t, typename Frame_t>
    void writeFrame(int it, FieldContainer_t& fc, ParticleContainer_t& pc, const Frame_t& frame,
                    const config& cfg, const Vector_t<int, Dim>& nr) {
        const int rank = ippl::Comm->rank();
        const int size = ippl::Comm->size();

        const int img_height = 400;
        const int img_width  = int(400.0 * cfg.extents[2] / cfg.extents[0]);
        const int floatcount = img_width * img_height * 3;

        std::vector<float> imagedata(floatcount, 0.0f);
        std::vector<float> recvbuffer(floatcount, 0.0f);
        ...
        for (size_t hi = 0; hi < nLocal; hi++) {
            ...
            if (y_imgcoord >= 0 && x_imgcoord >= 0 && x_imgcoord < img_width
                ...
                float& g = imagedata[(y_imgcoord * img_width + x_imgcoord) * 3 + 1];
                g        = std::min(255.f, g + intensity);
            }
        }

        // Color the lab-frame Poynting magnitude on the mid-y plane.
        auto eh = fc.getE().getHostMirror();
        auto bh = fc.getB().getHostMirror();
        Kokkos::deep_copy(eh, fc.getE().getView());
        Kokkos::deep_copy(bh, fc.getB().getView());
        ...
        for (int i = 1; i < img_width; i++) {
            for (int j = 1; j < img_height; j++) {
                ...
                if (i_remap >= ldom.first()[2] && i_remap <= ldom.last()[2]
                    && j_remap >= ldom.first()[0] && j_remap <= ldom.last()[0]) {
                    ippl::Vector<T, 3> E = eh(j_remap + 1 - ldom.first()[0], nr[1] / 2,
                                              i_remap + 1 - ldom.first()[2]);
                    ippl::Vector<T, 3> B = bh(j_remap + 1 - ldom.first()[0], nr[1] / 2,
                                              i_remap + 1 - ldom.first()[2]);
                    ...
                    float normalized = std::sqrt(poynting.Pnorm()) * 0.00001f;
                    int index        = (int)std::max(0.0f, std::min(normalized * 255.0f, 255.0f));
                    for (int c = 0; c < 3; c++) {
                        imagedata[(j * img_width + i) * 3 + c] += turbo_cm[index][c] * 255.0f;
                    }
                }
            }
        }

        // Butterfly reduction of the partial images onto rank 0.
        int mask = 1;
        while (mask < size) {
            int partner = rank ^ mask;
            if ((rank & mask) == 0) {
                MPI_Recv(recvbuffer.data(), floatcount, MPI_FLOAT, partner, 0,
                         ippl::Comm->getCommunicator(), MPI_STATUS_IGNORE);
                for (int f = 0; f < floatcount; f++) {
                    imagedata[f] += recvbuffer[f];
                }
            } else {
                MPI_Send(imagedata.data(), floatcount, MPI_FLOAT, partner, 0,
                         ippl::Comm->getCommunicator());
            }
            mask <<= 1;
        }

        if (rank == 0 && ffmpeg_file_ != nullptr) {
            std::vector<uint8_t> final_img(floatcount);
            std::transform(imagedata.begin(), imagedata.end(), final_img.begin(),
                           [](float x) { return (uint8_t)std::min(255.0f, std::max(0.0f, x)); });
            writeBMPToFD(ffmpeg_file_, img_width, img_height, final_img.data());
        }
        (void)it;
    }

private:
    FILE* ffmpeg_file_ = nullptr;
};
