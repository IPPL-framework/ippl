 ...
 * ```
 * srun ./LandauDamping <nx> [<ny>...] <Np> <Nt> <stype> <lbthres> <tintegr> --overallocate <ovfactor> --info 10
 * ```
 ...
 * ```
 * This command runs a simulation with a 128x128x128 grid, 10,000 particles, for 10 time steps, using the FFT solver,
 * a load balancing threshold of 1%, with LeapFrog integration, overallocating buffers by a factor of 2.
 * 
 * ### Implementation Detail
 * The main function initializes the Ippl framework, sets up the simulation parameters from command-line arguments,
 * and then creates and configures an instance of `LandauDampingManager` to manage the simulation. It performs pre-run
 * operations to set up the simulation environment and then runs the simulation for the specified number of time steps.
