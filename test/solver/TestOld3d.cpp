...
void testFEMSolver(const unsigned& numNodesPerDim, std::function<T(ippl::Vector<T, Dim> x)> f_rhs,
                   const T& domain_start = 0.0, const T& domain_end = 1.0) {
    ...
    if constexpr (Dim == 1) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const unsigned i, const unsigned j, const unsigned k) {
                ...
                sol.getView()(i, j, k) = analytic(x);
                rhs.getView()(i, j, k) = f_rhs(x);
            });
    }

    std::cout << "rhs view" << std::endl;
    rhs.write();
    ...
    error                 = lhs - sol;
    const double normError = norm(error) / norm(sol);
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        for (unsigned n = 1 << 2; n <= 1 << 2; n = n << 1) {
            testFEMSolver<T, 3>(n, gaussian3d<T>, 0.0, 1.0);
        }
