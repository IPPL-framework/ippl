...
void compute_convergence(char direction, unsigned int np, std::string fname) {
    ...
    const int dir = (direction == 'x') ? 0 : (direction == 'y') ? 1 : 2;

    // Specifie number of gridpoints in each direction (more gridpoints in z needed for CFL condition)
    ...
    // Initialize the source field with a Gaussian distribution in the given direction and zeros in the other directions
    ...
    Kokkos::parallel_for(ippl::getRangePolicy(aview, 1), KOKKOS_LAMBDA(size_t i, size_t j, size_t k){
        ...
        const scalar coord = (dir == 0) ? x : (dir == 1) ? y : z;
    ...
    // Run the simulation for 1s, with periodic boundary conditions this should be the same state as at time 0
    ...
    Kokkos::parallel_reduce(ippl::getRangePolicy(aview, 1), KOKKOS_LAMBDA(size_t i, size_t j, size_t k, double& ref){
        ...
        aview(i,j,k)[dir] -= gauss(scalar(0.5), scalar(0.05), coord);
        ref += Kokkos::sqrt(aview(i,j,k)[0] * aview(i,j,k)[0] + aview(i,j,k)[1] * aview(i,j,k)[1] + aview(i,j,k)[2] * aview(i,j,k)[2]);
    }, sum_error);
    ...
    csvout << direction << "," << np << "," << sum_error / double(nr[1] * nr[1] * nr[2]) << endl;
}
