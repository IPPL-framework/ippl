...
//   solution with anisotropic frequencies across dimensions.
//
...
//
// Usage:
//     ./TestMultigrid_convergence_periodic_anisotropic --info 5
//     ./TestMultigrid_convergence_periodic_anisotropic 8 --info 5
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        for (unsigned pt = 1u << 3; pt <= (1u << maxPow); pt = pt << 1) {
            ...
            const double ax_freq = 2.0 * pi;  // x: 1 wavelength
            const double ay_freq = 4.0 * pi;  // y: 2 wavelengths
            const double az_freq = 6.0 * pi;  // z: 3 wavelengths
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(const int i, const int j, const int k) {
                    ...
                    // RHS: -Δu with anisotropic frequencies
                    // Each term contributes: aᵢ² * (∂²u/∂xᵢ²)
                    const double f =
