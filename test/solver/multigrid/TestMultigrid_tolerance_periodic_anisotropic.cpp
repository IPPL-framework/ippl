...
//       x_i = (i + 0.5) h.
//
// Manufactured solution (anisotropic periodic frequencies):
//
//       u(x,y,z) = sin(sin(2*pi x)) sin(sin(4*pi y)) sin(sin(6*pi z)).
//
// Analytical RHS:
//
//       -Delta u = f.
//
// Usage:
//     ./TestMultigrid_convergence_periodic_aniso --info 5
//     ./TestMultigrid_convergence_periodic_aniso 8 --info 5
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        for (unsigned pt = 1u << 3; pt <= (1u << maxPow); pt = pt << 1) {
            ...
            // Different frequencies per dimension scaled for periodic boundaries
            const double ax_freq = 2.0 * pi;  // x: 1 periodic wavelength
            const double ay_freq = 4.0 * pi;  // y: 2 periodic wavelengths
            const double az_freq = 6.0 * pi;  // z: 3 periodic wavelengths
