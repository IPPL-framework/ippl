...
//
// Usage:
//     ./TestMultigrid_convergence_constant --info 5
//     ./TestMultigrid_convergence_constant 8 --info 5
//
// Optional first positional argument:
//     maxPow
//
// Default maxPow = 7, i.e. sizes 8^3, ..., 128^3.
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        // Solver configurations to run for each grid size.
        struct SolverConfig {
            const char* tag;
            const char* solverKind;          // "non-preconditioned" or "preconditioned"
            const char* preconditionerType;  // ignored if non-preconditioned
        };

        const std::array<SolverConfig, 5> configs = {{
            {"cg", "non-preconditioned", ""},
            {"pcg_gs", "preconditioned", "gauss-seidel"},
            {"pcg_ssor", "preconditioned", "ssor"},
            {"pcg_richardson", "preconditioned", "richardson"},
            {"mg_pcg", "preconditioned", "multigrid"},
        }};
        ...
        for (unsigned pt = 1u << 3; pt <= (1u << maxPow); pt = pt << 1) {
            ...
            for (const auto& cfg : configs) {
                ...
                params.add("solver", cfg.solverKind);

                if (std::string(cfg.solverKind) == "preconditioned") {
                    params.add("preconditioner_type", cfg.preconditionerType);
                ...
                m << cfg.tag << "," << pt << "," << std::setprecision(16) << dx << "," << relError
