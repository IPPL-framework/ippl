...
void testFEMSolver(const unsigned& numNodesPerDim, const T& domain_start = 0.0,
    ...
    if constexpr (Dim == 2) {
        Kokkos::parallel_for(
            "Assign RHS", rhsVector.size(), KOKKOS_LAMBDA(size_t i) {
                size_t nx = extents(0);
        ...
        Kokkos::parallel_for(
            "Assign RHS", rhsVector.size(), KOKKOS_LAMBDA(size_t i) {
                ...
                if (i - (nx * (ny - 1) + ny * (nx - 1) + nx * ny) * zOffset
                    >= (nx * (ny - 1) + ny * (nx - 1))) {
    ...
    Kokkos::parallel_for(
        "assign positions", numPoints - 3, KOKKOS_LAMBDA(size_t i) {
            (void)positions;
            (void)ldom;
            (void)domain_start;
            ...
            if constexpr (Dim == 2) {
                positions(i) = point_t(T(gen.drand(ldom.first()<:0:> * cellWidth + domain_start,
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        msg << std::setw(10) << "num_nodes" << std::setw(25) << "cell_spacing" << std::setw(25)
            << "interp_error_coef" << std::setw(25) << "recon_error" << std::setw(25)
            << "solver_residue" << std::setw(15) << "num_it" << endl;
