...
class Element {
public:
    KOKKOS_FUNCTION void function() const { printf("Element::function()\n"); }
};
...
public:
    ...
    void evaluateAx(F& eval) {
        ...
        Kokkos::parallel_for(
            "ClassA::execute", Kokkos::RangePolicy<>(0, 3), KOKKOS_CLASS_LAMBDA(int i) {
...
struct EvalFunctor {
    ...
    KOKKOS_FUNCTION int operator()(size_t i) const { return val * i; }
};

class FEMPoissonSolver {
public:
    FEMPoissonSolver(LagrangeSpace& space)
        : lagrange_space(space) {}
