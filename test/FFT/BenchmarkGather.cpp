/**
 * @file BenchmarkGather.cpp
 * @brief Benchmark for gather kernel implementations (Atomic, Tiled)
 *
 * Usage: ./BenchmarkGather [options]
...
// ============================================================================
// Gather Benchmark Implementation
// ============================================================================
...
public:
    ...
    GatherBenchmark(const benchmark::BenchmarkParams& params)
        : params_(params)
    ...
    void run_all_benchmarks() {
        benchmark::print_header("Gather Kernel Benchmark");
...
private:
    ...
    void initialize() {
        ...
        grid_input_ = std::make_unique<Field_t>(*mesh_, *layout_, nghost_);
    ...
    benchmark::TimingStats benchmark_config(const ippl::Interpolation::GatherConfig& cfg) {
        ...
        for (int i = 0; i < params_.warmup_runs; ++i) {
            ...
            bunch_->Q_result.gather(*grid_input_, bunch_->R, kernel_, false, cfg);
        }
    ...
    std::unique_ptr<Field_t> grid_input_;
...
int main(int argc, char* argv[]) {
...

    {
        ...
        GatherBenchmark<Kokkos::DefaultExecutionSpace> benchmark(params);
