...
inline TimingStats compute_stats(std::vector<double>& times_ms) {
    ...
    stats.stddev_ms = (stats.num_samples > 1)
                      ? std::sqrt(sq_sum / (stats.num_samples - 1))
                      : 0.0;
...
inline void print_stats_row(const std::string& label, const TimingStats& stats) {
    ...
    if (rank == 0) {
        std::cout << std::left << std::setw(25) << label
                  ...
                  << std::setw(10) << stats.mean_ms << " ms"
                  << " ± " << std::setw(8) << stats.stddev_ms << " ms"
                  << "  [min: " << std::setw(8) << stats.min_ms
                  << ", max: " << std::setw(8) << stats.max_ms
                  << ", med: " << std::setw(8) << stats.median_ms << "]\n";
    }
}
    ...
    int nghost)
{
    ...
    // Initialize grid data for gather benchmarks
