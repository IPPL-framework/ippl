...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        Kokkos::parallel_reduce(
            "NUDFT type2",
            mdrange_type({0, 0, 0},
                         {fview.extent(0) - 2 * nghost, fview.extent(1) - 2 * nghost,
        ...
        // double rel_error_imag = std::fabs(reducedValue.imag() - Q_result(idx).imag())
        ...
        //          << abs_error_imag << " Rel. error in imag part: " << std::setprecision(16)
        //          rel_error_imag << std::endl;
