...
void createParticleDistribution(Particles& P, std::string distribution, unsigned count, double qi,
    ...
    if (P->singleInitNode()) {
        ...
        if (type == UNIFORM) {
            ...
            double density =
                count * 3. / (sphere_radius * sphere_radius * sphere_radius * 4 * 3.14159);
        ...
        if (type == RANDOM) {
            ...
            // std::uniform_real_distribution<double> unidistribution(source[0]-sphere_radius,
            // source[0]+sphere_radius);
            ...
            for (unsigned i = 0; i < count; ++i) {
                ...
                double U = uni();
                Vektor<double, 3> pos =
                    source + sphere_radius * std::pow(U, 1. / 3.) / std::sqrt(dot(X, X)) * X;
                    ...
                    for (int iv = 0; iv < Nv[0]; ++iv) {
                        for (int jv = 0; jv < Nv[1]; ++jv) {
                            for (int kv = 0; kv < Nv[2]; ++kv) {
                                // double vx = (iv+.5)*hv+vmin; double vy = (jv+.5)*hv+vmin; double
                                // vz = (kv+.5)*hv+vmin;
                                Vektor<double, 3> vel =
                                    Vektor<double, 3>(iv + .5, jv + .5, kv + .5) * hv + Vmin;
                                double v2 = vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2];

                                double f = (1. / (30 * M_PI)) * exp(-0.5 * v2)
                    ...
                    for (int iv = 0; iv < Nv[0]; ++iv) {
                        for (int jv = 0; jv < Nv[1]; ++jv) {
                            for (int kv = 0; kv < Nv[2]; ++kv) {
                                ...
                                double f = (1. / (2. * M_PI * sqrt(2. * M_PI))) * exp(-0.5 * v2)
                                           * (1.
                                              + alpha
                                                    * (cos(kk * pos[2]) + cos(kk * pos[1])
                                                       + cos(kk * pos[0])));
                    ...
                    for (int iv = 0; iv < Nv[0]; ++iv) {
                        for (int jv = 0; jv < Nv[1]; ++jv) {
                            for (int kv = 0; kv < Nv[2]; ++kv) {
                                ...
                                // Free-streaming:
                                double f =
                                    alpha * (1. / (2. * M_PI * sqrt(2. * M_PI))) * exp(-0.5 * v2)
                                    * (cos(kk * pos[2]) + cos(kk * pos[1]) + cos(kk * pos[0]));
...
void createParticleDistributionMicrobunching(Particles& P, unsigned seedID = 0) {
    ...
    if (P->singleInitNode()) {
        ...
        for (unsigned i = 0; i < Nparticle; ++i) {
            ...
            /*
                                    for (int j=0;j<10;++j){
                                            //std::cout << std::exp(-imag*k) << P->b0[j] <<
               std::endl; P->b0[j]+=std::exp(-imag*k*(z+P->R[i][0]*sin(P->theta[j])));
                                    }
            */
        }
    }
...
void createParticleDistributionHeating(Particles& P, Vektor<double, 3> /*extend_l*/,
    ...
    if (P->singleInitNode()) {
        ...
        for (unsigned i = 0; i < Nparticle; ++i) {
            ...
            double U = uni();
            Vektor<double, 3> pos =
                source + beam_radius * std::pow(U, 1. / 3.) / std::sqrt(dot(X, X)) * X;
