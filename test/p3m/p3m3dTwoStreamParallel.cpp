...
public:
    ...
    void calculateGridForces(double /*interaction_radius*/, double alpha, double eps,
...
#ifdef SPHERE
        // rhocmpl_m[domain_m]=1./(nr_m[0]*nr_m[1]*nr_m[2])+rho_m[domain_m];
        rhocmpl_m[lDomain_m] = 1. / (hr_m[0] * hr_m[1] * hr_m[2] * (nr_m[0] * nr_m[1] * nr_m[2]))
                               + rhocmpl_m[lDomain_m];
#endif
        ...
        if (GcalcKSpace)
            ...
            // This loop stores in grnIField_m[i] the index of the ith dimension mirrored at the
    ...
    void operator()(std::size_t i, std::size_t j, ChargedParticles<playout_t>& P,
        ...
        if (sqr != 0) {
            ...
            if (P.Q[i] != 0 && P.Q[j] != 0) {
                ...
                // compute force
                Vector_t Fij = 1. / (4. * M_PI) * C * (diff / sqrt(sqr))
...
int main(int argc, char* argv[]) {
    ippl::initialize(argc, argv);
    {
        ...
        double Error, L2_exact, ErrorV, L2_V_exact, Total_E, Total_V, Total_E_exact,
            Total_V_exact;  // A, Potential_energy;
