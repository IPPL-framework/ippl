...
def read_gcc_xml_folder(folder: Path, test_run_id: str) -> List[TestResult]:
    ...
    for xml_file in folder.glob("*.xml"):
        test_results.extend(parse_ctest_xml_file(xml_file, test_run_id))
    return test_results

def parse_ctest_xml_file(xml_file: Path, test_run_id: str) -> List[TestResult]:
    ...
    for testcase in root.findall("testcase"):
        ...
        labels = [
            Label(name="suite", value=root.attrib.get("name", "gcc-xml-suite")),
