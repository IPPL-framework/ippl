...
public:
    ...
    static Space make_space(Mesh_t& mesh, Elem& e, Quad& q) { return Space(mesh, e, q); }

    // Axis-aligned domain bounds from mesh & nx
    static void domain_bounds(const Mesh_t& mesh, int nx, ippl::Vector<T, dim>& xmin,
    ...
    static double fill_uniform_particles(bunch_t& bunch, int n_local,
                                         ...
                                         const ippl::Vector<T, dim>& xmax, uint64_t seed = 1337) {
