...
TYPED_TEST(AssembleCurrentTest, SingleParticle_Smoke) {
    ...
    T dt = T(1.0);
    ippl::assemble_current_nedelec(mesh, bunch.Q, bunch.R, bunch.R_next, fem_vector, space, policy,
                                   dt);
    fem_vector.accumulateHalo();

    SUCCEED() << "assemble_current_nedelec ran without error for 1 particle in " << Dim << "D.";
}
...
TYPED_TEST(AssembleCurrentTest, SingleAxis_X_SameCell_ExactValues) {
    ...
    auto in_ldom = [&](const auto& cell) {
        for (unsigned d = 0; d < Dim; ++d)
            if (static_cast<int>(cell[d]) < ldom.first()[d]
                || static_cast<int>(cell[d]) > ldom.last()[d])
                return false;
        return true;
    };
...
TYPED_TEST(AssembleCurrentTest, DiagonalPath_ThreeCells_ExactValues) {
    ...
    if constexpr (Dim != 2) {
        ...
        ippl::assemble_current_nedelec(mesh, bunch.Q, bunch.R, bunch.R_next, fem_vector, space,
                                       policy, dt);
        fem_vector.accumulateHalo();
...
TYPED_TEST(AssembleCurrentTest, DiagonalPath_VertexHit_3D) {
    ...
    if constexpr (Dim != 3) {
        ...
        // Cell (1,1,1): identical to first cell, except different order as cell is mirrored across
        // vertex
        {
