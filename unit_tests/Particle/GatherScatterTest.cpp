...
template <typename ViewType, typename HashType>
struct ComputeTotalChargeLambda {
    ViewType viewQ;
    HashType hash;

    ComputeTotalChargeLambda(ViewType viewQ_, HashType hash_)
        : viewQ(viewQ_)
        , hash(hash_) {}

    KOKKOS_INLINE_FUNCTION void operator()(const size_t i, double& val) const {
        val += static_cast<double>(viewQ(hash(i)));
    }
};

template <typename ScatterType, typename FieldType>
double scatterConservationTolerance(const double reference) {
    using tolerance_type = std::conditional_t<(std::numeric_limits<ScatterType>::digits
                                               < std::numeric_limits<FieldType>::digits),
                                              ScatterType, FieldType>;
    return 100.0 * static_cast<double>(std::numeric_limits<tolerance_type>::epsilon())
...
//
// ScatterMixedValueTypeTest:
// Scatter a lower-precision particle attribute into a higher-precision field.
// This covers ParticleAttrib<float> -> Field<double>, which is the mixed type
// use case enabled by ParticleAttrib::scatter accepting a field value type that
// differs from the attribute value type.
//
TYPED_TEST(GatherScatterTest, ScatterMixedValueTypeTest) {
    using ScatterType = float;
    using FieldType   = double;

    const unsigned int n = this->nScatter;
    this->fillRandomPositions(n);
    this->fillAttributeQFloat(ScatterType(1.0));
...
//
// ScatterMixedValueTypeCustomHashTest:
// Scatter ParticleAttrib<float> into Field<double> through a custom hash array.
// This verifies that both the mixed value type and the hash-dispatched scatter
// path conserve the charge represented by the lower-precision attribute.
//
TYPED_TEST(GatherScatterTest, ScatterMixedValueTypeCustomHashTest) {
    using ScatterType = float;
    using FieldType   = double;
    ...
    using hash_type = typename TestFixture::bunch_type::mixed_charge_container_type::hash_type;
    hash_type hash("mixedIndexArray", nLoc);
    std::vector<int> host_indices(nLoc);
    std::iota(host_indices.begin(), host_indices.end(), 0);
    std::shuffle(host_indices.begin(), host_indices.end(), eng);
