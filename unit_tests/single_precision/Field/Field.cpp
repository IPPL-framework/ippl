...
TEST_F(FieldTest, Grad) {
    *field = 1.;
    ...
    ippl::Field<ippl::Vector<float, dim>, dim, mesh_type, centering_type> vfield(*mesh, *layout);
    vfield = grad(*field);
...
TEST_F(FieldTest, Curl) {
    ...
    ippl::Field<ippl::Vector<float, dim>, dim, mesh_type, centering_type> result(*mesh, *layout);
    result = curl(vfield);
...
TEST_F(FieldTest, Hessian) {
    ...
    ippl::Field<float, dim, mesh_type, centering_type> field(*mesh, *layout);
