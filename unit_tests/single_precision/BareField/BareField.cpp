...
public:
    ...
    typedef ippl::BareField<float, dim> field_type;
    typedef ippl::BareField<ippl::Vector<float, 3>, dim> vfield_type;
...
TEST_F(BareFieldTest, Sum) {
    ...
    *field = val;
    ...
    ASSERT_FLOAT_EQ(val * std::pow(nPoints, dim), sum);
}
...
TEST_F(BareFieldTest, Max) {
    ...
    float max = field->max();
    float expected = -1. + nPoints * 3;
    ASSERT_FLOAT_EQ(max, expected);
}

TEST_F(BareFieldTest, Prod) {
    *field = 2.;
    float val = field->prod();
    ASSERT_FLOAT_EQ(val, pow(2, nPoints * nPoints * nPoints));
}
...
TEST_F(BareFieldTest, ScalarMultiplication) {
    ...
    *field = *field * 10;
    ...
    for (size_t i = shift; i < mirror.extent(0) - shift; ++i) {
        for (size_t j = shift; j < mirror.extent(1) - shift; ++j) {
            for (size_t k = shift; k < mirror.extent(2) - shift; ++k) {
                ASSERT_FLOAT_EQ(mirror(i,j,k), 10.);
            }
        }
    }
}
...
TEST_F(BareFieldTest, DotProduct) {
    ...
    *field = 5. * dot(*vfield, *vfield);
    ...
    for (size_t i = shift; i < mirror.extent(0) - shift; ++i) {
        for (size_t j = shift; j < mirror.extent(1) - shift; ++j) {
            for (size_t k = shift; k < mirror.extent(2) - shift; ++k) {
                ASSERT_FLOAT_EQ(mirror(i,j,k), 15.);
            }
        }
    }
}
...
TEST_F(BareFieldTest, AllFuncs) {
    ...
    *field = alpha;
    // Compute new value
    float beta = fabs(7.0 * (sin(alpha) * cos(alpha))/(tan(alpha) * acos(alpha)) 
                - exp(alpha) + erf(alpha) + (asin(alpha) * cosh(alpha)) / (atan(alpha) 
                * sinh(alpha)) + tanh(alpha) * log(alpha)
    ...
    *field = fabs(7.0 * (sin(*field) * cos(*field))/(tan(*field) * acos(*field)) 
    ...
    for (size_t i = shift; i < mirror.extent(0) - shift; ++i) {
        for (size_t j = shift; j < mirror.extent(1) - shift; ++j) {
            for (size_t k = shift; k < mirror.extent(2) - shift; ++k) {
                ASSERT_FLOAT_EQ(mirror(i,j,k), beta);
            }
        }
    }
}
