...
public:
    ...
    typedef ippl::ParticleSpatialLayout<float, 3> playout_type;
    typedef ippl::detail::RegionLayout<float, 3, mesh_type> RegionLayout_t;
    ...
    void setup() {
        ...
        for (size_t i = 0; i < bunch->getLocalNum(); ++i) {
            ippl::Vector<float, dim> r = {unif(eng), unif(eng), unif(eng)};
            R_host(i)                   = r;
        }
