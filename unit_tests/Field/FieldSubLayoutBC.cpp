...
public:
    ...
    FieldBCTest()
        ...
        for (unsigned int d = 0; d < Dim; d++) {
            hx[d]     = domain[d] / (nPoints[d]+4);
            origin[d] = 0;
        }

        layout = ippl::SubFieldLayout<Dim>(MPI_COMM_WORLD, originOwned, owned, isParallel);
    ...
    ippl::SubFieldLayout<Dim> layout;
