...
namespace params {
    ...
    constexpr double nu =
        1.0;  // perp-parallel temperature anisotropy, ν = v_th_perp_i / v_th_par_i

    constexpr double D_D =
        1.0;  // Debye length (setting this to 1.0 is equivalent to setting L_ref = λ_D)
    // constexpr double D_C = 10.0;  // ion thermal gyroradius ρ_th_i, in units of L_ref. To have B
    // = 0, set D_C = ∞
    ...
    constexpr double f_x =
        0.1;  // resolution of the smallest length scale min(ρ_th_e, λ_D, ρ_th_i). should be < 1.0
    ...
    constexpr double v_trunc_i =
        6.0 * v_th_i;  // velocity at which to truncate the ion distribution function
    constexpr double v_trunc_e =
        6.0 * v_th_e;  // velocity at which to truncate the electron distribution function
    constexpr double f_ion_speedup =
        10.0;  // rough estimate of the velocity of the ions as the impact the wall

    // postprocessing of simulation parameters
    constexpr double dx0 =
        f_x
        * std::min({rho_th_e, rho_th_i, D_D});  // resolution such that dx << smallest length scale
    constexpr unsigned int nx = std::ceil(L / dx0);
    constexpr double dx       = L / (double)nx;  // the actual dx
    constexpr double v_max    = std::max(
        {v_trunc_e, f_ion_speedup* v_trunc_i});  // maximum velocity that we expect to encounter
    constexpr double dt = std::min({
        std::isfinite(D_C) ? (f_t * 2.0 * pi / std::max({Omega_ci, Omega_ce}))
